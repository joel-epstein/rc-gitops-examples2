...
Fig_63: {
	...
	config: [
		...
		// Fig_63 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_63 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-63/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-63$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-63
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-63 proxy object. All configs
