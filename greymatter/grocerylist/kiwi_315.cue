...
Kiwi_315: {
	...
	config: [
		...
		// Kiwi_315 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_315 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_315/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_315$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_315
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_315 proxy object. All configs
