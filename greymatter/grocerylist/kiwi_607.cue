...
Kiwi_607: {
	...
	config: [
		...
		// Kiwi_607 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_607 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_607/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_607$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_607
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_607 proxy object. All configs
