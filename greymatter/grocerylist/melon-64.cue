...
Melon_64: {
	...
	config: [
		...
		// Melon_64 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_64 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-64/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-64$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-64
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-64 proxy object. All configs
