...
Kiwi_531: {
	...
	config: [
		...
		// Kiwi_531 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_531 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_531/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_531$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_531
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_531 proxy object. All configs
