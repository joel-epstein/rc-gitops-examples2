...
Tangerine_6: {
	...
	config: [
		...
		// Tangerine_6 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Tangerine_6 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/tangerine-6/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/tangerine-6$"
					to:            route_match.path
		...
		// greymatter catalog service definition for tangerine-6
		#catalog_entry & {
			name:                      Name
		...
		// Shared tangerine-6 proxy object. All configs
