...
Fig_74: {
	...
	config: [
		...
		// Fig_74 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_74 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-74/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-74$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-74
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-74 proxy object. All configs
