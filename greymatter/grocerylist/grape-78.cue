...
Grape_78: {
	...
	config: [
		...
		// Grape_78 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_78 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-78/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-78$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-78
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-78 proxy object. All configs
