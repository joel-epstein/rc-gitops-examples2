...
Grape_75: {
	...
	config: [
		...
		// Grape_75 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_75 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-75/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-75$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-75
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-75 proxy object. All configs
