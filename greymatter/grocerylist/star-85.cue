...
Star_85: {
	...
	config: [
		...
		// Star_85 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_85 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-85/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-85$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-85
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-85 proxy object. All configs
