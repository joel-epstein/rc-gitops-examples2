...
Star_73: {
	...
	config: [
		...
		// Star_73 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_73 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-73/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-73$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-73
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-73 proxy object. All configs
