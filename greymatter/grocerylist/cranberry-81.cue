...
Cranberry_81: {
	...
	config: [
		...
		// Cranberry_81 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_81 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-81/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-81$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-81
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-81 proxy object. All configs
