...
Grape_56: {
	...
	config: [
		...
		// Grape_56 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_56 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-56/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-56$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-56
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-56 proxy object. All configs
