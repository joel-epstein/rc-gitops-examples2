...
Star_36: {
	...
	config: [
		...
		// Star_36 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_36 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-36/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-36$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-36
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-36 proxy object. All configs
