...
Blueberry_16: {
	...
	config: [
		...
		// Blueberry_16 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_16 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-16/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-16$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-16
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-16 proxy object. All configs
