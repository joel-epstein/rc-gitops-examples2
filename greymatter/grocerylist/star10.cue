...
Star10: {
	...
	config: [
		...
		// Star10 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star10 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star10/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star10$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star10
		#catalog_entry & {
			name:                      Name
		...
		// Shared star10 proxy object. All configs
