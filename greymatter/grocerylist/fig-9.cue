...
Fig_9: {
	...
	config: [
		...
		// Fig_9 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_9 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-9/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-9$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-9
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-9 proxy object. All configs
