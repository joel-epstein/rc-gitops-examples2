...
Star_89: {
	...
	config: [
		...
		// Star_89 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_89 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-89/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-89$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-89
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-89 proxy object. All configs
