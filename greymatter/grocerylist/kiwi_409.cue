...
Kiwi_409: {
	...
	config: [
		...
		// Kiwi_409 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_409 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_409/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_409$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_409
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_409 proxy object. All configs
