...
Star_86: {
	...
	config: [
		...
		// Star_86 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_86 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-86/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-86$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-86
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-86 proxy object. All configs
