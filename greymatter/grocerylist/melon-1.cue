...
Melon_1: {
	...
	config: [
		...
		// Melon_1 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_1 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-1/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-1$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-1
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-1 proxy object. All configs
