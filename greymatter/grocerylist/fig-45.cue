...
Fig_45: {
	...
	config: [
		...
		// Fig_45 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_45 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-45/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-45$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-45
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-45 proxy object. All configs
