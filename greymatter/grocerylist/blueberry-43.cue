...
Blueberry_43: {
	...
	config: [
		...
		// Blueberry_43 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_43 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-43/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-43$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-43
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-43 proxy object. All configs
