...
Grape_80: {
	...
	config: [
		...
		// Grape_80 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_80 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-80/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-80$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-80
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-80 proxy object. All configs
