...
Star_77: {
	...
	config: [
		...
		// Star_77 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_77 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-77/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-77$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-77
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-77 proxy object. All configs
