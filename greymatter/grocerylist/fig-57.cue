...
Fig_57: {
	...
	config: [
		...
		// Fig_57 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_57 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-57/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-57$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-57
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-57 proxy object. All configs
