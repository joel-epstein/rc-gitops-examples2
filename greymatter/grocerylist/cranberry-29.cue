...
Cranberry_29: {
	...
	config: [
		...
		// Cranberry_29 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_29 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-29/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-29$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-29
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-29 proxy object. All configs
