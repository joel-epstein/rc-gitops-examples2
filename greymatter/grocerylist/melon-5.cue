...
Melon_5: {
	...
	config: [
		...
		// Melon_5 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_5 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-5/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-5$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-5
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-5 proxy object. All configs
