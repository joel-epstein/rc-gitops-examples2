...
Tangerine_4: {
	...
	config: [
		...
		// Tangerine_4 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Tangerine_4 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/tangerine-4/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/tangerine-4$"
					to:            route_match.path
		...
		// greymatter catalog service definition for tangerine-4
		#catalog_entry & {
			name:                      Name
		...
		// Shared tangerine-4 proxy object. All configs
