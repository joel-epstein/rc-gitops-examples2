...
Star_32: {
	...
	config: [
		...
		// Star_32 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_32 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-32/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-32$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-32
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-32 proxy object. All configs
