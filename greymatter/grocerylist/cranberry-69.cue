...
Cranberry_69: {
	...
	config: [
		...
		// Cranberry_69 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_69 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-69/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-69$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-69
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-69 proxy object. All configs
