...
Cranberry50: {
	...
	config: [
		...
		// Cranberry50 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry50 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry50/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry50$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry50
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry50 proxy object. All configs
