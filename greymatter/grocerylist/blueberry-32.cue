...
Blueberry_32: {
	...
	config: [
		...
		// Blueberry_32 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_32 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-32/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-32$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-32
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-32 proxy object. All configs
