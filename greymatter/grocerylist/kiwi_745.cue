...
Kiwi_745: {
	...
	config: [
		...
		// Kiwi_745 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_745 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_745/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_745$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_745
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_745 proxy object. All configs
