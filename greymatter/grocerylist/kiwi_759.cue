...
Kiwi_759: {
	...
	config: [
		...
		// Kiwi_759 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_759 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_759/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_759$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_759
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_759 proxy object. All configs
