...
Cranberry_70: {
	...
	config: [
		...
		// Cranberry_70 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_70 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-70/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-70$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-70
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-70 proxy object. All configs
