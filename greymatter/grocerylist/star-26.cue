...
Star_26: {
	...
	config: [
		...
		// Star_26 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_26 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-26/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-26$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-26
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-26 proxy object. All configs
