...
Kiwi_1: {
	...
	config: [
		...
		// Kiwi_1 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_1 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-1/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-1$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-1
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-1 proxy object. All configs
