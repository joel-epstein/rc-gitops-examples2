...
Star_69: {
	...
	config: [
		...
		// Star_69 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_69 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-69/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-69$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-69
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-69 proxy object. All configs
