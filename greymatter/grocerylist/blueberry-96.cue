...
Blueberry_96: {
	...
	config: [
		...
		// Blueberry_96 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_96 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-96/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-96$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-96
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-96 proxy object. All configs
