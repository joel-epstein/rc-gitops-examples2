...
Fig_3: {
	...
	config: [
		...
		// Fig_3 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_3 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-3/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-3$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-3
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-3 proxy object. All configs
