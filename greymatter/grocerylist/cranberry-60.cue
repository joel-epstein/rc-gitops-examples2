...
Cranberry_60: {
	...
	config: [
		...
		// Cranberry_60 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_60 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-60/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-60$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-60
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-60 proxy object. All configs
