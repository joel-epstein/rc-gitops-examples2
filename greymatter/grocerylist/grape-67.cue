...
Grape_67: {
	...
	config: [
		...
		// Grape_67 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_67 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-67/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-67$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-67
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-67 proxy object. All configs
