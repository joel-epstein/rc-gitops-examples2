...
Fig_96: {
	...
	config: [
		...
		// Fig_96 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_96 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-96/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-96$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-96
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-96 proxy object. All configs
