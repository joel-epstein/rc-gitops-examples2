...
Orange_9: {
	...
	config: [
		...
		// Orange_9 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Orange_9 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/orange-9/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/orange-9$"
					to:            route_match.path
		...
		// greymatter catalog service definition for orange-9
		#catalog_entry & {
			name:                      Name
		...
		// Shared orange-9 proxy object. All configs
