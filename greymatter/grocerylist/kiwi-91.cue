...
Kiwi_91: {
	...
	config: [
		...
		// Kiwi_91 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_91 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-91/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-91$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-91
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-91 proxy object. All configs
