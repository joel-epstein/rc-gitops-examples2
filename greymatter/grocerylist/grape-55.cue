...
Grape_55: {
	...
	config: [
		...
		// Grape_55 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_55 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-55/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-55$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-55
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-55 proxy object. All configs
