...
Orange_2: {
	...
	config: [
		...
		// Orange_2 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Orange_2 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/orange-2/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/orange-2$"
					to:            route_match.path
		...
		// greymatter catalog service definition for orange-2
		#catalog_entry & {
			name:                      Name
		...
		// Shared orange-2 proxy object. All configs
