...
Melon_10: {
	...
	config: [
		...
		// Melon_10 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_10 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-10/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-10$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-10
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-10 proxy object. All configs
