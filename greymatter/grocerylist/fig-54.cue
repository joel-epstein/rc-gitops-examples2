...
Fig_54: {
	...
	config: [
		...
		// Fig_54 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_54 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-54/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-54$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-54
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-54 proxy object. All configs
