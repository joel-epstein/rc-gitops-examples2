...
Blueberry_89: {
	...
	config: [
		...
		// Blueberry_89 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_89 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-89/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-89$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-89
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-89 proxy object. All configs
