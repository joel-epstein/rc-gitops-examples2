...
Cranberry_7: {
	...
	config: [
		...
		// Cranberry_7 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_7 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-7/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-7$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-7
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-7 proxy object. All configs
