...
Cranberry_76: {
	...
	config: [
		...
		// Cranberry_76 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_76 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-76/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-76$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-76
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-76 proxy object. All configs
