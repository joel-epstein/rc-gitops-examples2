...
Kiwi_663: {
	...
	config: [
		...
		// Kiwi_663 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_663 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_663/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_663$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_663
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_663 proxy object. All configs
