...
Cranberry_94: {
	...
	config: [
		...
		// Cranberry_94 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_94 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-94/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-94$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-94
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-94 proxy object. All configs
