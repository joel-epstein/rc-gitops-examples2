...
Cranberry_67: {
	...
	config: [
		...
		// Cranberry_67 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_67 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-67/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-67$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-67
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-67 proxy object. All configs
