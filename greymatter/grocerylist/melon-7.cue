...
Melon_7: {
	...
	config: [
		...
		// Melon_7 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_7 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-7/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-7$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-7
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-7 proxy object. All configs
