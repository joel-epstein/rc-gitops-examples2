...
Cranberry49: {
	...
	config: [
		...
		// Cranberry49 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry49 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry49/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry49$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry49
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry49 proxy object. All configs
