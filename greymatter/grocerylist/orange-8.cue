...
Orange_8: {
	...
	config: [
		...
		// Orange_8 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Orange_8 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/orange-8/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/orange-8$"
					to:            route_match.path
		...
		// greymatter catalog service definition for orange-8
		#catalog_entry & {
			name:                      Name
		...
		// Shared orange-8 proxy object. All configs
