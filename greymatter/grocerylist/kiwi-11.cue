...
Kiwi_11: {
	...
	config: [
		...
		// Kiwi_11 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_11 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-11/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-11$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-11
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-11 proxy object. All configs
