...
Cranberry_36: {
	...
	config: [
		...
		// Cranberry_36 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_36 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-36/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-36$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-36
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-36 proxy object. All configs
