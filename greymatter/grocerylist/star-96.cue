...
Star_96: {
	...
	config: [
		...
		// Star_96 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_96 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-96/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-96$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-96
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-96 proxy object. All configs
