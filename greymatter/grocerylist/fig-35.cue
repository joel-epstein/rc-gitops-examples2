...
Fig_35: {
	...
	config: [
		...
		// Fig_35 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_35 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-35/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-35$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-35
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-35 proxy object. All configs
