...
Kiwi_832: {
	...
	config: [
		...
		// Kiwi_832 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_832 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_832/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_832$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_832
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_832 proxy object. All configs
