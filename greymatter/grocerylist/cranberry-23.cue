...
Cranberry_23: {
	...
	config: [
		...
		// Cranberry_23 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_23 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-23/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-23$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-23
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-23 proxy object. All configs
