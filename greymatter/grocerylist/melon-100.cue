...
Melon_100: {
	...
	config: [
		...
		// Melon_100 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_100 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-100/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-100$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-100
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-100 proxy object. All configs
