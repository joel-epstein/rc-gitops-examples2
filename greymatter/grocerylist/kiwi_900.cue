...
Kiwi_900: {
	...
	config: [
		...
		// Kiwi_900 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_900 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_900/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_900$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_900
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_900 proxy object. All configs
