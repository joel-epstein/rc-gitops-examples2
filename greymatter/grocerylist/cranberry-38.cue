...
Cranberry_38: {
	...
	config: [
		...
		// Cranberry_38 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_38 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-38/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-38$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-38
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-38 proxy object. All configs
