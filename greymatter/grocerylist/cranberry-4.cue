...
Cranberry_4: {
	...
	config: [
		...
		// Cranberry_4 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_4 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-4/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-4$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-4
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-4 proxy object. All configs
