...
Fig_46: {
	...
	config: [
		...
		// Fig_46 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_46 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-46/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-46$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-46
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-46 proxy object. All configs
