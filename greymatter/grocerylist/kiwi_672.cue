...
Kiwi_672: {
	...
	config: [
		...
		// Kiwi_672 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_672 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_672/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_672$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_672
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_672 proxy object. All configs
