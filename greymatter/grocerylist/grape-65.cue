...
Grape_65: {
	...
	config: [
		...
		// Grape_65 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_65 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-65/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-65$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-65
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-65 proxy object. All configs
