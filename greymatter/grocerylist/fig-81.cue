...
Fig_81: {
	...
	config: [
		...
		// Fig_81 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_81 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-81/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-81$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-81
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-81 proxy object. All configs
