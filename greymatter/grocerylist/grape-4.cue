...
Grape_4: {
	...
	config: [
		...
		// Grape_4 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_4 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-4/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-4$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-4
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-4 proxy object. All configs
