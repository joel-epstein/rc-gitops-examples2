...
Cranberry_99: {
	...
	config: [
		...
		// Cranberry_99 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_99 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-99/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-99$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-99
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-99 proxy object. All configs
