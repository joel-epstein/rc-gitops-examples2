...
Star_95: {
	...
	config: [
		...
		// Star_95 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_95 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-95/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-95$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-95
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-95 proxy object. All configs
