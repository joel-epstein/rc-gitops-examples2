...
Star_72: {
	...
	config: [
		...
		// Star_72 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_72 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-72/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-72$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-72
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-72 proxy object. All configs
