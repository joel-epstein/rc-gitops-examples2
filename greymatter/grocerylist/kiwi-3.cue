...
Kiwi_3: {
	...
	config: [
		...
		// Kiwi_3 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_3 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-3/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-3$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-3
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-3 proxy object. All configs
