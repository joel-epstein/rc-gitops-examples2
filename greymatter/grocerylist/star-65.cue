...
Star_65: {
	...
	config: [
		...
		// Star_65 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_65 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-65/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-65$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-65
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-65 proxy object. All configs
