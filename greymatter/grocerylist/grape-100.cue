...
Grape_100: {
	...
	config: [
		...
		// Grape_100 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_100 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-100/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-100$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-100
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-100 proxy object. All configs
