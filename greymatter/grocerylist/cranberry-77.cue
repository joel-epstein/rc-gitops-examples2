...
Cranberry_77: {
	...
	config: [
		...
		// Cranberry_77 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_77 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-77/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-77$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-77
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-77 proxy object. All configs
