...
Star_2: {
	...
	config: [
		...
		// Star_2 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_2 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-2/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-2$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-2
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-2 proxy object. All configs
