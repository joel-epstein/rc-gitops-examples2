...
Star_52: {
	...
	config: [
		...
		// Star_52 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_52 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-52/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-52$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-52
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-52 proxy object. All configs
