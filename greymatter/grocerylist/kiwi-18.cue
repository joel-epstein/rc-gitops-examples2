...
Kiwi_18: {
	...
	config: [
		...
		// Kiwi_18 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_18 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-18/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-18$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-18
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-18 proxy object. All configs
