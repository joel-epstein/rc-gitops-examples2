...
Fig_62: {
	...
	config: [
		...
		// Fig_62 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_62 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-62/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-62$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-62
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-62 proxy object. All configs
