...
Cranberry_96: {
	...
	config: [
		...
		// Cranberry_96 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_96 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-96/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-96$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-96
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-96 proxy object. All configs
