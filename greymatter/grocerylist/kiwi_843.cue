...
Kiwi_843: {
	...
	config: [
		...
		// Kiwi_843 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_843 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_843/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_843$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_843
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_843 proxy object. All configs
