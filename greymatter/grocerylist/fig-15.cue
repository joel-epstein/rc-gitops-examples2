...
Fig_15: {
	...
	config: [
		...
		// Fig_15 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_15 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-15/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-15$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-15
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-15 proxy object. All configs
