...
Star_64: {
	...
	config: [
		...
		// Star_64 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_64 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-64/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-64$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-64
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-64 proxy object. All configs
