...
Fig_97: {
	...
	config: [
		...
		// Fig_97 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_97 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-97/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-97$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-97
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-97 proxy object. All configs
