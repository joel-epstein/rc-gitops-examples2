...
Star_43: {
	...
	config: [
		...
		// Star_43 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_43 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-43/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-43$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-43
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-43 proxy object. All configs
