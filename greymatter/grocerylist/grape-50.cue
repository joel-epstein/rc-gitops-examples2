...
Grape_50: {
	...
	config: [
		...
		// Grape_50 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_50 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-50/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-50$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-50
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-50 proxy object. All configs
