...
Grape_90: {
	...
	config: [
		...
		// Grape_90 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_90 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-90/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-90$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-90
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-90 proxy object. All configs
