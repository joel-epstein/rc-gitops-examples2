...
Cranberry_61: {
	...
	config: [
		...
		// Cranberry_61 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_61 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-61/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-61$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-61
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-61 proxy object. All configs
