...
Cranberry_93: {
	...
	config: [
		...
		// Cranberry_93 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_93 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-93/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-93$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-93
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-93 proxy object. All configs
