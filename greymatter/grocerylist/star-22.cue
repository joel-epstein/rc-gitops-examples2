...
Star_22: {
	...
	config: [
		...
		// Star_22 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_22 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-22/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-22$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-22
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-22 proxy object. All configs
