...
Fig_31: {
	...
	config: [
		...
		// Fig_31 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_31 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-31/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-31$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-31
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-31 proxy object. All configs
