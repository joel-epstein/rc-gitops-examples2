...
Grape_24: {
	...
	config: [
		...
		// Grape_24 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_24 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-24/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-24$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-24
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-24 proxy object. All configs
