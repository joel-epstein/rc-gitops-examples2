...
Cranberry_43: {
	...
	config: [
		...
		// Cranberry_43 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_43 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-43/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-43$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-43
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-43 proxy object. All configs
