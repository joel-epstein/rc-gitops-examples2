...
Cranberry_2: {
	...
	config: [
		...
		// Cranberry_2 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_2 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-2/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-2$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-2
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-2 proxy object. All configs
