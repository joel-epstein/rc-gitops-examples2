...
Star_68: {
	...
	config: [
		...
		// Star_68 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_68 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-68/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-68$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-68
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-68 proxy object. All configs
