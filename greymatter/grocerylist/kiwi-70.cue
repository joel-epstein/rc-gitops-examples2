...
Kiwi_70: {
	...
	config: [
		...
		// Kiwi_70 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_70 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-70/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-70$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-70
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-70 proxy object. All configs
