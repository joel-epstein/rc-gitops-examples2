...
Kiwi_561: {
	...
	config: [
		...
		// Kiwi_561 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_561 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_561/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_561$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_561
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_561 proxy object. All configs
