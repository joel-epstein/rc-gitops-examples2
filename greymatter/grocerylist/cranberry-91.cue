...
Cranberry_91: {
	...
	config: [
		...
		// Cranberry_91 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_91 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-91/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-91$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-91
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-91 proxy object. All configs
