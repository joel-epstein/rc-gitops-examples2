...
Orange_88: {
	...
	config: [
		...
		// Orange_88 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Orange_88 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/orange-88/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/orange-88$"
					to:            route_match.path
		...
		// greymatter catalog service definition for orange-88
		#catalog_entry & {
			name:                      Name
		...
		// Shared orange-88 proxy object. All configs
