...
Kiwi_16: {
	...
	config: [
		...
		// Kiwi_16 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_16 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-16/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-16$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-16
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-16 proxy object. All configs
