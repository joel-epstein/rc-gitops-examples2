...
Star_40: {
	...
	config: [
		...
		// Star_40 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_40 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-40/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-40$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-40
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-40 proxy object. All configs
