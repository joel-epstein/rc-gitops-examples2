...
Kiwi_751: {
	...
	config: [
		...
		// Kiwi_751 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_751 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_751/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_751$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_751
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_751 proxy object. All configs
