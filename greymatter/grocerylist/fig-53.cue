...
Fig_53: {
	...
	config: [
		...
		// Fig_53 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_53 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-53/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-53$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-53
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-53 proxy object. All configs
