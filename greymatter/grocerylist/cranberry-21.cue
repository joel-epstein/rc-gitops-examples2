...
Cranberry_21: {
	...
	config: [
		...
		// Cranberry_21 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_21 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-21/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-21$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-21
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-21 proxy object. All configs
