...
Grape_59: {
	...
	config: [
		...
		// Grape_59 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_59 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-59/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-59$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-59
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-59 proxy object. All configs
