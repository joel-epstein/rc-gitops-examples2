...
Orange_78: {
	...
	config: [
		...
		// Orange_78 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Orange_78 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/orange-78/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/orange-78$"
					to:            route_match.path
		...
		// greymatter catalog service definition for orange-78
		#catalog_entry & {
			name:                      Name
		...
		// Shared orange-78 proxy object. All configs
