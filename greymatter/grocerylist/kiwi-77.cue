...
Kiwi_77: {
	...
	config: [
		...
		// Kiwi_77 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_77 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-77/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-77$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-77
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-77 proxy object. All configs
