...
Kiwi_998: {
	...
	config: [
		...
		// Kiwi_998 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_998 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_998/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_998$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_998
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_998 proxy object. All configs
