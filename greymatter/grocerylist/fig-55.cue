...
Fig_55: {
	...
	config: [
		...
		// Fig_55 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_55 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-55/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-55$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-55
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-55 proxy object. All configs
