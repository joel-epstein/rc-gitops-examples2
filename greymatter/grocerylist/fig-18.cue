...
Fig_18: {
	...
	config: [
		...
		// Fig_18 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_18 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-18/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-18$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-18
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-18 proxy object. All configs
