...
Kiwi_10: {
	...
	config: [
		...
		// Kiwi_10 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_10 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-10/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-10$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-10
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-10 proxy object. All configs
