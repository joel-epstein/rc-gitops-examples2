...
Star_45: {
	...
	config: [
		...
		// Star_45 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_45 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-45/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-45$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-45
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-45 proxy object. All configs
