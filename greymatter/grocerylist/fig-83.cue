...
Fig_83: {
	...
	config: [
		...
		// Fig_83 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_83 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-83/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-83$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-83
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-83 proxy object. All configs
