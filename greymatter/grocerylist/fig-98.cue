...
Fig_98: {
	...
	config: [
		...
		// Fig_98 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_98 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-98/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-98$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-98
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-98 proxy object. All configs
