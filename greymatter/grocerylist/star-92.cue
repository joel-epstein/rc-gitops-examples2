...
Star_92: {
	...
	config: [
		...
		// Star_92 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_92 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-92/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-92$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-92
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-92 proxy object. All configs
