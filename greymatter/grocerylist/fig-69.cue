...
Fig_69: {
	...
	config: [
		...
		// Fig_69 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_69 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-69/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-69$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-69
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-69 proxy object. All configs
