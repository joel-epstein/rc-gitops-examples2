...
Blueberry_5: {
	...
	config: [
		...
		// Blueberry_5 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_5 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-5/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-5$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-5
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-5 proxy object. All configs
