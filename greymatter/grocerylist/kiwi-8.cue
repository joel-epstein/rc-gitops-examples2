...
Kiwi_8: {
	...
	config: [
		...
		// Kiwi_8 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_8 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-8/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-8$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-8
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-8 proxy object. All configs
