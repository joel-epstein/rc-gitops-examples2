...
Fig_32: {
	...
	config: [
		...
		// Fig_32 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_32 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-32/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-32$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-32
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-32 proxy object. All configs
