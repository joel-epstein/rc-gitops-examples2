...
Melon_61: {
	...
	config: [
		...
		// Melon_61 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_61 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-61/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-61$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-61
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-61 proxy object. All configs
