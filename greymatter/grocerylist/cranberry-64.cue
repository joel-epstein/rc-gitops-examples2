...
Cranberry_64: {
	...
	config: [
		...
		// Cranberry_64 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_64 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-64/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-64$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-64
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-64 proxy object. All configs
