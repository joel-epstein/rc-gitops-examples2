...
Cranberry_78: {
	...
	config: [
		...
		// Cranberry_78 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_78 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-78/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-78$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-78
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-78 proxy object. All configs
