...
Star_8: {
	...
	config: [
		...
		// Star_8 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_8 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-8/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-8$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-8
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-8 proxy object. All configs
