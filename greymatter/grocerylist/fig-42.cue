...
Fig_42: {
	...
	config: [
		...
		// Fig_42 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_42 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-42/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-42$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-42
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-42 proxy object. All configs
