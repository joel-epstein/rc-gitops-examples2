...
Kiwi_849: {
	...
	config: [
		...
		// Kiwi_849 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_849 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_849/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_849$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_849
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_849 proxy object. All configs
