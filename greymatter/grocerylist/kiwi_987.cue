...
Kiwi_987: {
	...
	config: [
		...
		// Kiwi_987 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_987 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_987/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_987$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_987
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_987 proxy object. All configs
