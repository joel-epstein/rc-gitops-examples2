...
Blueberry_56: {
	...
	config: [
		...
		// Blueberry_56 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_56 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-56/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-56$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-56
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-56 proxy object. All configs
