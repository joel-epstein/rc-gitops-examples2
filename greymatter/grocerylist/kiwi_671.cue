...
Kiwi_671: {
	...
	config: [
		...
		// Kiwi_671 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_671 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_671/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_671$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_671
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_671 proxy object. All configs
