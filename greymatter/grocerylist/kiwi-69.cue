...
Kiwi_69: {
	...
	config: [
		...
		// Kiwi_69 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_69 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-69/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-69$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-69
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-69 proxy object. All configs
