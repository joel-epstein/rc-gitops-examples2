...
Melon_55: {
	...
	config: [
		...
		// Melon_55 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_55 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-55/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-55$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-55
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-55 proxy object. All configs
