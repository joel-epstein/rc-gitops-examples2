...
Grape_1: {
	...
	config: [
		...
		// Grape_1 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_1 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-1/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-1$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-1
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-1 proxy object. All configs
