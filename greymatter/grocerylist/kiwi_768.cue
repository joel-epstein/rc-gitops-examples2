...
Kiwi_768: {
	...
	config: [
		...
		// Kiwi_768 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_768 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_768/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_768$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_768
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_768 proxy object. All configs
