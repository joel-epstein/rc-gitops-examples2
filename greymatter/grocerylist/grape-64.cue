...
Grape_64: {
	...
	config: [
		...
		// Grape_64 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_64 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-64/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-64$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-64
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-64 proxy object. All configs
