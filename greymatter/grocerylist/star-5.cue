...
Star_5: {
	...
	config: [
		...
		// Star_5 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_5 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-5/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-5$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-5
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-5 proxy object. All configs
