...
Cranberry_50: {
	...
	config: [
		...
		// Cranberry_50 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_50 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-50/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-50$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-50
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-50 proxy object. All configs
