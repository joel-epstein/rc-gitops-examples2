...
Cranberry_49: {
	...
	config: [
		...
		// Cranberry_49 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_49 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-49/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-49$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-49
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-49 proxy object. All configs
