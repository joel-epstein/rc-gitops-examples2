...
Melon_94: {
	...
	config: [
		...
		// Melon_94 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_94 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-94/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-94$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-94
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-94 proxy object. All configs
