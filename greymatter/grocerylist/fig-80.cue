...
Fig_80: {
	...
	config: [
		...
		// Fig_80 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_80 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-80/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-80$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-80
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-80 proxy object. All configs
