...
Grape_89: {
	...
	config: [
		...
		// Grape_89 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_89 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-89/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-89$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-89
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-89 proxy object. All configs
