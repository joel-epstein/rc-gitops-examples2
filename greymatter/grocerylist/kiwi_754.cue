...
Kiwi_754: {
	...
	config: [
		...
		// Kiwi_754 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_754 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_754/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_754$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_754
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_754 proxy object. All configs
