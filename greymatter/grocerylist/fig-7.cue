...
Fig_7: {
	...
	config: [
		...
		// Fig_7 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_7 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-7/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-7$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-7
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-7 proxy object. All configs
