...
Grape_96: {
	...
	config: [
		...
		// Grape_96 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_96 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-96/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-96$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-96
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-96 proxy object. All configs
