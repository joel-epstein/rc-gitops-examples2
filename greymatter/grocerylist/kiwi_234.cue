...
Kiwi_234: {
	...
	config: [
		...
		// Kiwi_234 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_234 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_234/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_234$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_234
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_234 proxy object. All configs
