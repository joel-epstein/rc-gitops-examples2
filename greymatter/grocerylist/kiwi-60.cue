...
Kiwi_60: {
	...
	config: [
		...
		// Kiwi_60 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_60 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-60/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-60$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-60
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-60 proxy object. All configs
