...
Star_38: {
	...
	config: [
		...
		// Star_38 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_38 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-38/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-38$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-38
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-38 proxy object. All configs
