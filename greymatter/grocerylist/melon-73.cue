...
Melon_73: {
	...
	config: [
		...
		// Melon_73 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_73 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-73/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-73$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-73
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-73 proxy object. All configs
