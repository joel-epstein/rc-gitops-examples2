...
Blueberry_24: {
	...
	config: [
		...
		// Blueberry_24 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_24 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-24/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-24$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-24
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-24 proxy object. All configs
