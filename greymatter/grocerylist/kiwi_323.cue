...
Kiwi_323: {
	...
	config: [
		...
		// Kiwi_323 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_323 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_323/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_323$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_323
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_323 proxy object. All configs
