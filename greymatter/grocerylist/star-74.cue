...
Star_74: {
	...
	config: [
		...
		// Star_74 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_74 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-74/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-74$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-74
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-74 proxy object. All configs
