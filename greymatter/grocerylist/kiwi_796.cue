...
Kiwi_796: {
	...
	config: [
		...
		// Kiwi_796 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_796 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_796/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_796$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_796
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_796 proxy object. All configs
