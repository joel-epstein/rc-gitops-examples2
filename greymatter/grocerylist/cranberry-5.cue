...
Cranberry_5: {
	...
	config: [
		...
		// Cranberry_5 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_5 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-5/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-5$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-5
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-5 proxy object. All configs
