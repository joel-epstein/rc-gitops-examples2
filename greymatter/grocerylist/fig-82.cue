...
Fig_82: {
	...
	config: [
		...
		// Fig_82 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_82 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-82/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-82$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-82
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-82 proxy object. All configs
