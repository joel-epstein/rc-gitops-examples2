...
Blueberry_80: {
	...
	config: [
		...
		// Blueberry_80 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_80 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-80/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-80$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-80
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-80 proxy object. All configs
