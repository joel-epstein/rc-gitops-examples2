...
Kiwi_94: {
	...
	config: [
		...
		// Kiwi_94 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_94 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-94/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-94$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-94
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-94 proxy object. All configs
