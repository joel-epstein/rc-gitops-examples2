...
Fig_4: {
	...
	config: [
		...
		// Fig_4 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_4 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-4/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-4$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-4
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-4 proxy object. All configs
