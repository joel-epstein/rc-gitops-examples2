...
Blueberry_3: {
	...
	config: [
		...
		// Blueberry_3 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_3 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-3/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-3$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-3
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-3 proxy object. All configs
