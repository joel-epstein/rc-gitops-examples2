...
Fig_11: {
	...
	config: [
		...
		// Fig_11 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_11 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-11/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-11$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-11
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-11 proxy object. All configs
