...
Star_63: {
	...
	config: [
		...
		// Star_63 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_63 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-63/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-63$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-63
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-63 proxy object. All configs
