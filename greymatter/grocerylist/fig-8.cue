...
Fig_8: {
	...
	config: [
		...
		// Fig_8 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_8 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-8/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-8$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-8
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-8 proxy object. All configs
