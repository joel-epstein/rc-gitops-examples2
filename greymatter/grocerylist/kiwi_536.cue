...
Kiwi_536: {
	...
	config: [
		...
		// Kiwi_536 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_536 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_536/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_536$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_536
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_536 proxy object. All configs
