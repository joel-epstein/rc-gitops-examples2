...
Fig_23: {
	...
	config: [
		...
		// Fig_23 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_23 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-23/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-23$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-23
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-23 proxy object. All configs
