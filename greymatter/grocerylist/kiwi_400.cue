...
Kiwi_400: {
	...
	config: [
		...
		// Kiwi_400 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_400 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_400/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_400$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_400
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_400 proxy object. All configs
