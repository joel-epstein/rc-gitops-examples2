...
Fig_87: {
	...
	config: [
		...
		// Fig_87 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_87 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-87/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-87$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-87
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-87 proxy object. All configs
