...
Kiwi_447: {
	...
	config: [
		...
		// Kiwi_447 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_447 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_447/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_447$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_447
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_447 proxy object. All configs
