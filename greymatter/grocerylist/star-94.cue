...
Star_94: {
	...
	config: [
		...
		// Star_94 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_94 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-94/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-94$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-94
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-94 proxy object. All configs
