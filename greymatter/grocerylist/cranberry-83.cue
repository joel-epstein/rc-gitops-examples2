...
Cranberry_83: {
	...
	config: [
		...
		// Cranberry_83 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_83 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-83/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-83$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-83
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-83 proxy object. All configs
