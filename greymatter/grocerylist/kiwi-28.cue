...
Kiwi_28: {
	...
	config: [
		...
		// Kiwi_28 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_28 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-28/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-28$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-28
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-28 proxy object. All configs
