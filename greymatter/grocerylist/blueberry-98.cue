...
Blueberry_98: {
	...
	config: [
		...
		// Blueberry_98 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_98 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-98/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-98$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-98
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-98 proxy object. All configs
