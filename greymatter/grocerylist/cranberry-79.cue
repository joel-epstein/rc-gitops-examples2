...
Cranberry_79: {
	...
	config: [
		...
		// Cranberry_79 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_79 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-79/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-79$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-79
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-79 proxy object. All configs
