...
Kiwi_40: {
	...
	config: [
		...
		// Kiwi_40 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_40 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-40/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-40$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-40
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-40 proxy object. All configs
