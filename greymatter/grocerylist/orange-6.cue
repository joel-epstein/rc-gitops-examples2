...
Orange_6: {
	...
	config: [
		...
		// Orange_6 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Orange_6 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/orange-6/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/orange-6$"
					to:            route_match.path
		...
		// greymatter catalog service definition for orange-6
		#catalog_entry & {
			name:                      Name
		...
		// Shared orange-6 proxy object. All configs
