...
Blueberry_6: {
	...
	config: [
		...
		// Blueberry_6 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_6 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-6/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-6$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-6
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-6 proxy object. All configs
