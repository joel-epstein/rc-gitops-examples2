...
Fig_41: {
	...
	config: [
		...
		// Fig_41 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_41 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-41/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-41$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-41
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-41 proxy object. All configs
