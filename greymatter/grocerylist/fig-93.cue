...
Fig_93: {
	...
	config: [
		...
		// Fig_93 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_93 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-93/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-93$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-93
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-93 proxy object. All configs
