...
Fig_6: {
	...
	config: [
		...
		// Fig_6 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_6 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-6/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-6$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-6
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-6 proxy object. All configs
