...
Star_66: {
	...
	config: [
		...
		// Star_66 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_66 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-66/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-66$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-66
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-66 proxy object. All configs
