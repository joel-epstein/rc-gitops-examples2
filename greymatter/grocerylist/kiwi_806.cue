...
Kiwi_806: {
	...
	config: [
		...
		// Kiwi_806 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_806 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_806/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_806$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_806
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_806 proxy object. All configs
