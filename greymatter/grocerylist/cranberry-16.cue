...
Cranberry_16: {
	...
	config: [
		...
		// Cranberry_16 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_16 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-16/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-16$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-16
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-16 proxy object. All configs
