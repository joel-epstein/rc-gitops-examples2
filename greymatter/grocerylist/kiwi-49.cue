...
Kiwi_49: {
	...
	config: [
		...
		// Kiwi_49 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_49 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-49/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-49$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-49
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-49 proxy object. All configs
