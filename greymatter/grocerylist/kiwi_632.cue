...
Kiwi_632: {
	...
	config: [
		...
		// Kiwi_632 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_632 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_632/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_632$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_632
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_632 proxy object. All configs
