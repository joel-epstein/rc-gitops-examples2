...
Kiwi_88: {
	...
	config: [
		...
		// Kiwi_88 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_88 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-88/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-88$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-88
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-88 proxy object. All configs
