...
Fig_13: {
	...
	config: [
		...
		// Fig_13 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_13 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-13/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-13$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-13
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-13 proxy object. All configs
