...
Cranberry_95: {
	...
	config: [
		...
		// Cranberry_95 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_95 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-95/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-95$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-95
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-95 proxy object. All configs
