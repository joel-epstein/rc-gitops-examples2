...
Cranberry_42: {
	...
	config: [
		...
		// Cranberry_42 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_42 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-42/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-42$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-42
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-42 proxy object. All configs
