...
Melon_8: {
	...
	config: [
		...
		// Melon_8 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_8 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-8/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-8$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-8
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-8 proxy object. All configs
