...
Kiwi_467: {
	...
	config: [
		...
		// Kiwi_467 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_467 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_467/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_467$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_467
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_467 proxy object. All configs
