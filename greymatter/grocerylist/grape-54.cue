...
Grape_54: {
	...
	config: [
		...
		// Grape_54 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_54 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-54/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-54$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-54
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-54 proxy object. All configs
