...
Blueberry_50: {
	...
	config: [
		...
		// Blueberry_50 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_50 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-50/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-50$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-50
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-50 proxy object. All configs
