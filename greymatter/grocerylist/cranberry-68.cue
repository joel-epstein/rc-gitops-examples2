...
Cranberry_68: {
	...
	config: [
		...
		// Cranberry_68 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_68 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-68/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-68$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-68
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-68 proxy object. All configs
