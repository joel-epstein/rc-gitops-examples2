...
Star_80: {
	...
	config: [
		...
		// Star_80 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_80 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-80/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-80$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-80
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-80 proxy object. All configs
