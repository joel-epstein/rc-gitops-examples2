...
Cranberry_39: {
	...
	config: [
		...
		// Cranberry_39 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_39 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-39/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-39$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-39
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-39 proxy object. All configs
