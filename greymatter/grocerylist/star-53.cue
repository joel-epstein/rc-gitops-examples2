...
Star_53: {
	...
	config: [
		...
		// Star_53 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_53 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-53/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-53$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-53
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-53 proxy object. All configs
