...
Cranberry_71: {
	...
	config: [
		...
		// Cranberry_71 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_71 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-71/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-71$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-71
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-71 proxy object. All configs
