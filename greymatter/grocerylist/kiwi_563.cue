...
Kiwi_563: {
	...
	config: [
		...
		// Kiwi_563 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_563 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_563/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_563$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_563
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_563 proxy object. All configs
