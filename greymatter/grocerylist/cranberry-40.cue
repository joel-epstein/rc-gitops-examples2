...
Cranberry_40: {
	...
	config: [
		...
		// Cranberry_40 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_40 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-40/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-40$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-40
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-40 proxy object. All configs
