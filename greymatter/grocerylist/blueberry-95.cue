...
Blueberry_95: {
	...
	config: [
		...
		// Blueberry_95 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_95 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-95/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-95$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-95
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-95 proxy object. All configs
