...
Cranberry_13: {
	...
	config: [
		...
		// Cranberry_13 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_13 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-13/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-13$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-13
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-13 proxy object. All configs
