...
Cranberry_65: {
	...
	config: [
		...
		// Cranberry_65 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_65 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-65/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-65$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-65
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-65 proxy object. All configs
