...
Kiwi_535: {
	...
	config: [
		...
		// Kiwi_535 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_535 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_535/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_535$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_535
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_535 proxy object. All configs
