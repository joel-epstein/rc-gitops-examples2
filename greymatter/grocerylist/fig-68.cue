...
Fig_68: {
	...
	config: [
		...
		// Fig_68 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_68 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-68/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-68$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-68
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-68 proxy object. All configs
