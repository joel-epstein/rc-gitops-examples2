...
Blueberry_55: {
	...
	config: [
		...
		// Blueberry_55 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_55 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-55/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-55$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-55
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-55 proxy object. All configs
