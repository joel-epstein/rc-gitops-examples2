...
Star_34: {
	...
	config: [
		...
		// Star_34 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_34 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-34/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-34$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-34
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-34 proxy object. All configs
