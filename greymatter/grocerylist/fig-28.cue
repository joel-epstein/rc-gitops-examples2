...
Fig_28: {
	...
	config: [
		...
		// Fig_28 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_28 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-28/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-28$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-28
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-28 proxy object. All configs
