...
Star_11: {
	...
	config: [
		...
		// Star_11 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_11 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-11/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-11$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-11
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-11 proxy object. All configs
