...
Kiwi_489: {
	...
	config: [
		...
		// Kiwi_489 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_489 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_489/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_489$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_489
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_489 proxy object. All configs
