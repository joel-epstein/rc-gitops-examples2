...
Star_60: {
	...
	config: [
		...
		// Star_60 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_60 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-60/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-60$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-60
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-60 proxy object. All configs
