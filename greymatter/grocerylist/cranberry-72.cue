...
Cranberry_72: {
	...
	config: [
		...
		// Cranberry_72 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_72 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-72/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-72$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-72
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-72 proxy object. All configs
