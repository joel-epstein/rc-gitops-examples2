...
Melon_50: {
	...
	config: [
		...
		// Melon_50 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_50 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-50/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-50$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-50
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-50 proxy object. All configs
