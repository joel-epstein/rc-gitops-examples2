...
Blueberry_1: {
	...
	config: [
		...
		// Blueberry_1 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_1 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-1/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-1$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-1
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-1 proxy object. All configs
