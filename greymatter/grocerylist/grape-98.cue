...
Grape_98: {
	...
	config: [
		...
		// Grape_98 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_98 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-98/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-98$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-98
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-98 proxy object. All configs
