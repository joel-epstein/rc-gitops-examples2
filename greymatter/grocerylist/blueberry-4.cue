...
Blueberry_4: {
	...
	config: [
		...
		// Blueberry_4 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_4 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-4/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-4$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-4
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-4 proxy object. All configs
