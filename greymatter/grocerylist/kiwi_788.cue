...
Kiwi_788: {
	...
	config: [
		...
		// Kiwi_788 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_788 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_788/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_788$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_788
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_788 proxy object. All configs
