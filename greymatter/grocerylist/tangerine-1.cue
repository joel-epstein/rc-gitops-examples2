...
Tangerine_1: {
	...
	config: [
		...
		// Tangerine_1 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Tangerine_1 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/tangerine-1/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/tangerine-1$"
					to:            route_match.path
		...
		// greymatter catalog service definition for tangerine-1
		#catalog_entry & {
			name:                      Name
		...
		// Shared tangerine-1 proxy object. All configs
