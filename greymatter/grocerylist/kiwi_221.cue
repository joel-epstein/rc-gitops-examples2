...
Kiwi_221: {
	...
	config: [
		...
		// Kiwi_221 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_221 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_221/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_221$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_221
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_221 proxy object. All configs
