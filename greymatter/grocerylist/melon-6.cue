...
Melon_6: {
	...
	config: [
		...
		// Melon_6 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_6 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-6/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-6$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-6
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-6 proxy object. All configs
