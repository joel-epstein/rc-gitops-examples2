...
Cranberry_24: {
	...
	config: [
		...
		// Cranberry_24 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_24 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-24/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-24$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-24
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-24 proxy object. All configs
