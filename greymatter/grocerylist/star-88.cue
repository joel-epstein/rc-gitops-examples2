...
Star_88: {
	...
	config: [
		...
		// Star_88 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_88 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-88/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-88$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-88
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-88 proxy object. All configs
