...
Star46: {
	...
	config: [
		...
		// Star46 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star46 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star46/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star46$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star46
		#catalog_entry & {
			name:                      Name
		...
		// Shared star46 proxy object. All configs
