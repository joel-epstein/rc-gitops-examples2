...
Kiwi_913: {
	...
	config: [
		...
		// Kiwi_913 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_913 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_913/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_913$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_913
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_913 proxy object. All configs
