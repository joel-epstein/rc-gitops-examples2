...
Star_41: {
	...
	config: [
		...
		// Star_41 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_41 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-41/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-41$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-41
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-41 proxy object. All configs
