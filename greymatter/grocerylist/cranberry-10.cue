...
Cranberry_10: {
	...
	config: [
		...
		// Cranberry_10 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_10 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-10/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-10$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-10
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-10 proxy object. All configs
