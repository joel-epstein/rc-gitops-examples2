...
Grape_25: {
	...
	config: [
		...
		// Grape_25 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_25 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-25/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-25$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-25
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-25 proxy object. All configs
