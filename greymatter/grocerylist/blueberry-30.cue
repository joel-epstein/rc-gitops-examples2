...
Blueberry_30: {
	...
	config: [
		...
		// Blueberry_30 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_30 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-30/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-30$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-30
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-30 proxy object. All configs
