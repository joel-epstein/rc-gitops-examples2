...
Cranberry_22: {
	...
	config: [
		...
		// Cranberry_22 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_22 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-22/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-22$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-22
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-22 proxy object. All configs
