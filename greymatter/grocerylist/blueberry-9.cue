...
Blueberry_9: {
	...
	config: [
		...
		// Blueberry_9 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_9 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-9/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-9$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-9
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-9 proxy object. All configs
