...
Kiwi_650: {
	...
	config: [
		...
		// Kiwi_650 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_650 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_650/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_650$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_650
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_650 proxy object. All configs
