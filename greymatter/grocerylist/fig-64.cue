...
Fig_64: {
	...
	config: [
		...
		// Fig_64 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_64 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-64/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-64$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-64
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-64 proxy object. All configs
