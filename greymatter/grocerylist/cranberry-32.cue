...
Cranberry_32: {
	...
	config: [
		...
		// Cranberry_32 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_32 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-32/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-32$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-32
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-32 proxy object. All configs
