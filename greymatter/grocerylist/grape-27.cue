...
Grape_27: {
	...
	config: [
		...
		// Grape_27 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_27 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-27/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-27$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-27
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-27 proxy object. All configs
