...
Star_24: {
	...
	config: [
		...
		// Star_24 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_24 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-24/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-24$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-24
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-24 proxy object. All configs
