...
Fig_48: {
	...
	config: [
		...
		// Fig_48 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_48 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-48/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-48$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-48
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-48 proxy object. All configs
