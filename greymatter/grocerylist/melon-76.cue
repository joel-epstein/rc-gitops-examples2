...
Melon_76: {
	...
	config: [
		...
		// Melon_76 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_76 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-76/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-76$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-76
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-76 proxy object. All configs
