...
Cranberry_26: {
	...
	config: [
		...
		// Cranberry_26 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_26 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-26/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-26$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-26
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-26 proxy object. All configs
