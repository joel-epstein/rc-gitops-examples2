...
Kiwi_42: {
	...
	config: [
		...
		// Kiwi_42 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_42 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-42/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-42$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-42
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-42 proxy object. All configs
