...
Star_93: {
	...
	config: [
		...
		// Star_93 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_93 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-93/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-93$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-93
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-93 proxy object. All configs
