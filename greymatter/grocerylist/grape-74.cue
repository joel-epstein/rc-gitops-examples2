...
Grape_74: {
	...
	config: [
		...
		// Grape_74 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_74 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-74/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-74$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-74
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-74 proxy object. All configs
