...
Blueberry_74: {
	...
	config: [
		...
		// Blueberry_74 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_74 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-74/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-74$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-74
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-74 proxy object. All configs
