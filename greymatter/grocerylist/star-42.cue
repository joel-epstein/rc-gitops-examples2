...
Star_42: {
	...
	config: [
		...
		// Star_42 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_42 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-42/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-42$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-42
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-42 proxy object. All configs
