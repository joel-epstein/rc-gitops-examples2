...
Cranberry_63: {
	...
	config: [
		...
		// Cranberry_63 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_63 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-63/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-63$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-63
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-63 proxy object. All configs
