...
Grape_73: {
	...
	config: [
		...
		// Grape_73 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_73 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-73/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-73$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-73
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-73 proxy object. All configs
