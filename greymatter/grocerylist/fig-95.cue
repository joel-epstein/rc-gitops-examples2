...
Fig_95: {
	...
	config: [
		...
		// Fig_95 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_95 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-95/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-95$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-95
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-95 proxy object. All configs
