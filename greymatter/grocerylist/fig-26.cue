...
Fig_26: {
	...
	config: [
		...
		// Fig_26 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_26 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-26/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-26$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-26
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-26 proxy object. All configs
