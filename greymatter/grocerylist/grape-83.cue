...
Grape_83: {
	...
	config: [
		...
		// Grape_83 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_83 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-83/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-83$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-83
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-83 proxy object. All configs
