...
Cranberry_15: {
	...
	config: [
		...
		// Cranberry_15 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_15 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-15/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-15$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-15
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-15 proxy object. All configs
