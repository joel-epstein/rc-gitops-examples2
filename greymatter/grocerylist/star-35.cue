...
Star_35: {
	...
	config: [
		...
		// Star_35 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_35 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-35/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-35$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-35
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-35 proxy object. All configs
