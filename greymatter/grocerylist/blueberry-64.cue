...
Blueberry_64: {
	...
	config: [
		...
		// Blueberry_64 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_64 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-64/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-64$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-64
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-64 proxy object. All configs
