...
Cranberry_97: {
	...
	config: [
		...
		// Cranberry_97 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_97 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-97/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-97$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-97
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-97 proxy object. All configs
