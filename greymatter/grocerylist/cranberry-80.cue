...
Cranberry_80: {
	...
	config: [
		...
		// Cranberry_80 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_80 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-80/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-80$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-80
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-80 proxy object. All configs
