...
Kiwi_881: {
	...
	config: [
		...
		// Kiwi_881 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_881 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_881/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_881$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_881
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_881 proxy object. All configs
