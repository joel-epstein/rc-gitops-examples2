...
Fig_19: {
	...
	config: [
		...
		// Fig_19 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_19 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-19/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-19$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-19
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-19 proxy object. All configs
