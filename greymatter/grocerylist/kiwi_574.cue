...
Kiwi_574: {
	...
	config: [
		...
		// Kiwi_574 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_574 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_574/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_574$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_574
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_574 proxy object. All configs
