...
Cranberry_92: {
	...
	config: [
		...
		// Cranberry_92 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_92 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-92/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-92$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-92
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-92 proxy object. All configs
