...
Star_97: {
	...
	config: [
		...
		// Star_97 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_97 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-97/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-97$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-97
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-97 proxy object. All configs
