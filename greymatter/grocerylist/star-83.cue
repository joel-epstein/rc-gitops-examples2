...
Star_83: {
	...
	config: [
		...
		// Star_83 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_83 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-83/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-83$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-83
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-83 proxy object. All configs
