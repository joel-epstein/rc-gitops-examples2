...
Fig_65: {
	...
	config: [
		...
		// Fig_65 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_65 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-65/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-65$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-65
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-65 proxy object. All configs
