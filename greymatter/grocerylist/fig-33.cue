...
Fig_33: {
	...
	config: [
		...
		// Fig_33 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_33 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-33/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-33$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-33
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-33 proxy object. All configs
