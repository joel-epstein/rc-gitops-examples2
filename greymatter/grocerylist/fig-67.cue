...
Fig_67: {
	...
	config: [
		...
		// Fig_67 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_67 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-67/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-67$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-67
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-67 proxy object. All configs
