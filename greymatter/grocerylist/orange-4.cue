...
Orange_4: {
	...
	config: [
		...
		// Orange_4 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Orange_4 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/orange-4/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/orange-4$"
					to:            route_match.path
		...
		// greymatter catalog service definition for orange-4
		#catalog_entry & {
			name:                      Name
		...
		// Shared orange-4 proxy object. All configs
