...
Cranberry_28: {
	...
	config: [
		...
		// Cranberry_28 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_28 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-28/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-28$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-28
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-28 proxy object. All configs
