...
Fig_30: {
	...
	config: [
		...
		// Fig_30 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_30 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-30/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-30$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-30
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-30 proxy object. All configs
