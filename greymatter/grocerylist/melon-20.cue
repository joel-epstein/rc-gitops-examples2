...
Melon_20: {
	...
	config: [
		...
		// Melon_20 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_20 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-20/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-20$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-20
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-20 proxy object. All configs
