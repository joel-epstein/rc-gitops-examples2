...
Melon_3: {
	...
	config: [
		...
		// Melon_3 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_3 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-3/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-3$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-3
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-3 proxy object. All configs
