...
Fig_79: {
	...
	config: [
		...
		// Fig_79 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_79 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-79/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-79$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-79
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-79 proxy object. All configs
