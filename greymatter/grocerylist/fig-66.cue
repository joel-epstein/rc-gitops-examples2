...
Fig_66: {
	...
	config: [
		...
		// Fig_66 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_66 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-66/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-66$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-66
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-66 proxy object. All configs
