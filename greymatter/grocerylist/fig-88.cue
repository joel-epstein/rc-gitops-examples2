...
Fig_88: {
	...
	config: [
		...
		// Fig_88 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_88 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-88/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-88$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-88
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-88 proxy object. All configs
