...
Orange_5: {
	...
	config: [
		...
		// Orange_5 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Orange_5 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/orange-5/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/orange-5$"
					to:            route_match.path
		...
		// greymatter catalog service definition for orange-5
		#catalog_entry & {
			name:                      Name
		...
		// Shared orange-5 proxy object. All configs
