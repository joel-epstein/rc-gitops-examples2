...
Blueberry_8: {
	...
	config: [
		...
		// Blueberry_8 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_8 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-8/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-8$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-8
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-8 proxy object. All configs
