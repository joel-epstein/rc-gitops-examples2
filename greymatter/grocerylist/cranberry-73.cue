...
Cranberry_73: {
	...
	config: [
		...
		// Cranberry_73 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_73 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-73/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-73$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-73
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-73 proxy object. All configs
