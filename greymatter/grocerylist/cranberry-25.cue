...
Cranberry_25: {
	...
	config: [
		...
		// Cranberry_25 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_25 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-25/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-25$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-25
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-25 proxy object. All configs
