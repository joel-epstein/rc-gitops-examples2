...
Kiwi_420: {
	...
	config: [
		...
		// Kiwi_420 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_420 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_420/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_420$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_420
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_420 proxy object. All configs
