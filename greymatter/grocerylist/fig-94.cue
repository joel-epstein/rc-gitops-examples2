...
Fig_94: {
	...
	config: [
		...
		// Fig_94 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_94 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-94/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-94$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-94
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-94 proxy object. All configs
