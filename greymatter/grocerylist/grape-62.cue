...
Grape_62: {
	...
	config: [
		...
		// Grape_62 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_62 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-62/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-62$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-62
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-62 proxy object. All configs
