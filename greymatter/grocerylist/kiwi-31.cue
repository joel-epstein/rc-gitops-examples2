...
Kiwi_31: {
	...
	config: [
		...
		// Kiwi_31 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_31 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-31/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-31$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-31
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-31 proxy object. All configs
