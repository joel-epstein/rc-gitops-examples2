...
Cranberry_41: {
	...
	config: [
		...
		// Cranberry_41 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_41 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-41/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-41$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-41
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-41 proxy object. All configs
