...
Kiwi_814: {
	...
	config: [
		...
		// Kiwi_814 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_814 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_814/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_814$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_814
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_814 proxy object. All configs
