...
Fig_90: {
	...
	config: [
		...
		// Fig_90 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_90 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-90/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-90$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-90
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-90 proxy object. All configs
