...
Fig_50: {
	...
	config: [
		...
		// Fig_50 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_50 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-50/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-50$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-50
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-50 proxy object. All configs
