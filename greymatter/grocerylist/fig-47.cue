...
Fig_47: {
	...
	config: [
		...
		// Fig_47 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_47 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-47/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-47$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-47
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-47 proxy object. All configs
