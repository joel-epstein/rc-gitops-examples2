...
Fig_84: {
	...
	config: [
		...
		// Fig_84 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_84 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-84/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-84$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-84
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-84 proxy object. All configs
