...
Star_4: {
	...
	config: [
		...
		// Star_4 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_4 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-4/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-4$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-4
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-4 proxy object. All configs
