...
Grape_3: {
	...
	config: [
		...
		// Grape_3 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_3 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-3/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-3$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-3
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-3 proxy object. All configs
