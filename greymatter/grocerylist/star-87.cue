...
Star_87: {
	...
	config: [
		...
		// Star_87 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_87 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-87/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-87$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-87
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-87 proxy object. All configs
