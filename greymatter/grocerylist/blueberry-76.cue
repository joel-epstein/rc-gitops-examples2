...
Blueberry_76: {
	...
	config: [
		...
		// Blueberry_76 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_76 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-76/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-76$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-76
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-76 proxy object. All configs
