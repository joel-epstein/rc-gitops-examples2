...
Kiwi_73: {
	...
	config: [
		...
		// Kiwi_73 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_73 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-73/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-73$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-73
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-73 proxy object. All configs
