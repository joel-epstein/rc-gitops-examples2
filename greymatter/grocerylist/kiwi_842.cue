...
Kiwi_842: {
	...
	config: [
		...
		// Kiwi_842 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_842 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_842/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_842$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_842
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_842 proxy object. All configs
