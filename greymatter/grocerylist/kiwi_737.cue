...
Kiwi_737: {
	...
	config: [
		...
		// Kiwi_737 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_737 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_737/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_737$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_737
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_737 proxy object. All configs
