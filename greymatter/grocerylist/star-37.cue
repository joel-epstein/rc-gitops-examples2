...
Star_37: {
	...
	config: [
		...
		// Star_37 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_37 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-37/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-37$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-37
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-37 proxy object. All configs
