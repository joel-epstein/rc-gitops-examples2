...
Kiwi_907: {
	...
	config: [
		...
		// Kiwi_907 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_907 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_907/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_907$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_907
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_907 proxy object. All configs
