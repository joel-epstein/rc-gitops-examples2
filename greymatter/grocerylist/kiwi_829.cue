...
Kiwi_829: {
	...
	config: [
		...
		// Kiwi_829 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_829 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_829/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_829$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_829
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_829 proxy object. All configs
