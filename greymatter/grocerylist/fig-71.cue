...
Fig_71: {
	...
	config: [
		...
		// Fig_71 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_71 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-71/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-71$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-71
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-71 proxy object. All configs
