...
Fig_43: {
	...
	config: [
		...
		// Fig_43 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_43 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-43/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-43$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-43
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-43 proxy object. All configs
