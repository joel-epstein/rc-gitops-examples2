...
Star_76: {
	...
	config: [
		...
		// Star_76 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_76 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-76/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-76$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-76
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-76 proxy object. All configs
