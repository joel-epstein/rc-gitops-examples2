...
Fig_61: {
	...
	config: [
		...
		// Fig_61 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_61 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-61/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-61$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-61
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-61 proxy object. All configs
