...
Fig_100: {
	...
	config: [
		...
		// Fig_100 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_100 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-100/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-100$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-100
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-100 proxy object. All configs
