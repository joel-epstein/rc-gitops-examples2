...
Star_9: {
	...
	config: [
		...
		// Star_9 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_9 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-9/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-9$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-9
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-9 proxy object. All configs
