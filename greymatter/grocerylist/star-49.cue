...
Star_49: {
	...
	config: [
		...
		// Star_49 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_49 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-49/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-49$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-49
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-49 proxy object. All configs
