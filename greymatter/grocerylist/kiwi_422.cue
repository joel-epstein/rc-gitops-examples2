...
Kiwi_422: {
	...
	config: [
		...
		// Kiwi_422 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_422 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_422/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_422$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_422
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_422 proxy object. All configs
