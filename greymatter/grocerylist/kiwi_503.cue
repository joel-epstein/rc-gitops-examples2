...
Kiwi_503: {
	...
	config: [
		...
		// Kiwi_503 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_503 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_503/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_503$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_503
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_503 proxy object. All configs
