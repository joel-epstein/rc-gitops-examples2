...
Star_98: {
	...
	config: [
		...
		// Star_98 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_98 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-98/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-98$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-98
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-98 proxy object. All configs
