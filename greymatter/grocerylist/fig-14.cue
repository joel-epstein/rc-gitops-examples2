...
Fig_14: {
	...
	config: [
		...
		// Fig_14 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_14 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-14/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-14$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-14
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-14 proxy object. All configs
