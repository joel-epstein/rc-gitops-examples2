...
Kiwi_100: {
	...
	config: [
		...
		// Kiwi_100 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_100 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-100/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-100$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-100
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-100 proxy object. All configs
