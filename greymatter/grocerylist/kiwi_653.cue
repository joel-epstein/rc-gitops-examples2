...
Kiwi_653: {
	...
	config: [
		...
		// Kiwi_653 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_653 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_653/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_653$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_653
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_653 proxy object. All configs
