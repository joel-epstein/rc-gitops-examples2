...
Star_14: {
	...
	config: [
		...
		// Star_14 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_14 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-14/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-14$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-14
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-14 proxy object. All configs
