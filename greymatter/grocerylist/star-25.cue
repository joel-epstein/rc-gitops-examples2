...
Star_25: {
	...
	config: [
		...
		// Star_25 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_25 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-25/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-25$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-25
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-25 proxy object. All configs
