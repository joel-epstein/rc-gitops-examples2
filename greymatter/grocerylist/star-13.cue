...
Star_13: {
	...
	config: [
		...
		// Star_13 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_13 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-13/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-13$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-13
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-13 proxy object. All configs
