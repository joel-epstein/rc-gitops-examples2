...
Cranberry_100: {
	...
	config: [
		...
		// Cranberry_100 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_100 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-100/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-100$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-100
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-100 proxy object. All configs
