...
Fig_89: {
	...
	config: [
		...
		// Fig_89 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_89 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-89/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-89$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-89
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-89 proxy object. All configs
