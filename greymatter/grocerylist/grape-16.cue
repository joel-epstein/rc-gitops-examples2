...
Grape_16: {
	...
	config: [
		...
		// Grape_16 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_16 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-16/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-16$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-16
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-16 proxy object. All configs
