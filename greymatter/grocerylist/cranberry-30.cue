...
Cranberry_30: {
	...
	config: [
		...
		// Cranberry_30 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_30 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-30/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-30$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-30
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-30 proxy object. All configs
