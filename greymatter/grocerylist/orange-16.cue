...
Orange_16: {
	...
	config: [
		...
		// Orange_16 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Orange_16 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/orange-16/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/orange-16$"
					to:            route_match.path
		...
		// greymatter catalog service definition for orange-16
		#catalog_entry & {
			name:                      Name
		...
		// Shared orange-16 proxy object. All configs
