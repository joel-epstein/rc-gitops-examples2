...
Grape_20: {
	...
	config: [
		...
		// Grape_20 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_20 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-20/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-20$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-20
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-20 proxy object. All configs
