...
Star_79: {
	...
	config: [
		...
		// Star_79 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_79 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-79/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-79$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-79
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-79 proxy object. All configs
