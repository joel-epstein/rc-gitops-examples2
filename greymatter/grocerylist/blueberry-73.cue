...
Blueberry_73: {
	...
	config: [
		...
		// Blueberry_73 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_73 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-73/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-73$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-73
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-73 proxy object. All configs
