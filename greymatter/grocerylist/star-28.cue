...
Star_28: {
	...
	config: [
		...
		// Star_28 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_28 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-28/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-28$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-28
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-28 proxy object. All configs
