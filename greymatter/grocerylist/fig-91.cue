...
Fig_91: {
	...
	config: [
		...
		// Fig_91 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_91 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-91/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-91$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-91
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-91 proxy object. All configs
