...
Star_27: {
	...
	config: [
		...
		// Star_27 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_27 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-27/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-27$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-27
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-27 proxy object. All configs
