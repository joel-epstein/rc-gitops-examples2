...
Grape_91: {
	...
	config: [
		...
		// Grape_91 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_91 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-91/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-91$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-91
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-91 proxy object. All configs
