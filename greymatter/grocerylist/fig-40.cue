...
Fig_40: {
	...
	config: [
		...
		// Fig_40 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_40 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-40/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-40$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-40
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-40 proxy object. All configs
