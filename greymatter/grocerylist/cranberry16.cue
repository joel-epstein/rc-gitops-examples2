...
Cranberry16: {
	...
	config: [
		...
		// Cranberry16 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry16 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry16/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry16$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry16
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry16 proxy object. All configs
