...
Star_99: {
	...
	config: [
		...
		// Star_99 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_99 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-99/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-99$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-99
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-99 proxy object. All configs
