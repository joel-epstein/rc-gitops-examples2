...
Fig_20: {
	...
	config: [
		...
		// Fig_20 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_20 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-20/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-20$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-20
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-20 proxy object. All configs
