...
Grape_6: {
	...
	config: [
		...
		// Grape_6 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_6 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-6/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-6$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-6
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-6 proxy object. All configs
