...
Blueberry_10: {
	...
	config: [
		...
		// Blueberry_10 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_10 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-10/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-10$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-10
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-10 proxy object. All configs
