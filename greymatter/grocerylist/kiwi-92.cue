...
Kiwi_92: {
	...
	config: [
		...
		// Kiwi_92 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_92 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-92/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-92$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-92
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-92 proxy object. All configs
