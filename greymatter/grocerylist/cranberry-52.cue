...
Cranberry_52: {
	...
	config: [
		...
		// Cranberry_52 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_52 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-52/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-52$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-52
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-52 proxy object. All configs
