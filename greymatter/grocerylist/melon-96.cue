...
Melon_96: {
	...
	config: [
		...
		// Melon_96 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_96 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-96/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-96$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-96
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-96 proxy object. All configs
