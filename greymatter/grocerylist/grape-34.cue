...
Grape_34: {
	...
	config: [
		...
		// Grape_34 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_34 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-34/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-34$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-34
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-34 proxy object. All configs
