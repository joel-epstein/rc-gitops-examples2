...
Melon_2: {
	...
	config: [
		...
		// Melon_2 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_2 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-2/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-2$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-2
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-2 proxy object. All configs
