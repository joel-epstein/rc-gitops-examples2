...
Star_31: {
	...
	config: [
		...
		// Star_31 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_31 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-31/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-31$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-31
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-31 proxy object. All configs
