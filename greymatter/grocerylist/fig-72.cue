...
Fig_72: {
	...
	config: [
		...
		// Fig_72 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_72 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-72/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-72$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-72
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-72 proxy object. All configs
