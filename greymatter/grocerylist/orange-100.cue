...
Orange_100: {
	...
	config: [
		...
		// Orange_100 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Orange_100 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/orange-100/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/orange-100$"
					to:            route_match.path
		...
		// greymatter catalog service definition for orange-100
		#catalog_entry & {
			name:                      Name
		...
		// Shared orange-100 proxy object. All configs
