...
Grape_12: {
	...
	config: [
		...
		// Grape_12 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_12 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-12/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-12$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-12
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-12 proxy object. All configs
