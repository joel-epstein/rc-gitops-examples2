...
Cranberry_82: {
	...
	config: [
		...
		// Cranberry_82 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_82 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-82/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-82$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-82
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-82 proxy object. All configs
