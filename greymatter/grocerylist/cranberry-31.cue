...
Cranberry_31: {
	...
	config: [
		...
		// Cranberry_31 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_31 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-31/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-31$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-31
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-31 proxy object. All configs
