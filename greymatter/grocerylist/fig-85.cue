...
Fig_85: {
	...
	config: [
		...
		// Fig_85 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_85 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-85/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-85$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-85
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-85 proxy object. All configs
