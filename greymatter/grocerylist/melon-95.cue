...
Melon_95: {
	...
	config: [
		...
		// Melon_95 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_95 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-95/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-95$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-95
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-95 proxy object. All configs
