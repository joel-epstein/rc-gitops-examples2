...
Fig_78: {
	...
	config: [
		...
		// Fig_78 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_78 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-78/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-78$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-78
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-78 proxy object. All configs
