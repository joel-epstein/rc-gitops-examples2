...
Star_20: {
	...
	config: [
		...
		// Star_20 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_20 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-20/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-20$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-20
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-20 proxy object. All configs
