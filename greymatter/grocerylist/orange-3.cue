...
Orange_3: {
	...
	config: [
		...
		// Orange_3 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Orange_3 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/orange-3/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/orange-3$"
					to:            route_match.path
		...
		// greymatter catalog service definition for orange-3
		#catalog_entry & {
			name:                      Name
		...
		// Shared orange-3 proxy object. All configs
