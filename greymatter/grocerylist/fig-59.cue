...
Fig_59: {
	...
	config: [
		...
		// Fig_59 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_59 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-59/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-59$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-59
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-59 proxy object. All configs
