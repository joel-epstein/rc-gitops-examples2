...
Star_10: {
	...
	config: [
		...
		// Star_10 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_10 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-10/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-10$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-10
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-10 proxy object. All configs
