...
Kiwi_480: {
	...
	config: [
		...
		// Kiwi_480 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_480 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_480/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_480$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_480
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_480 proxy object. All configs
