...
Kiwi_855: {
	...
	config: [
		...
		// Kiwi_855 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_855 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_855/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_855$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_855
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_855 proxy object. All configs
