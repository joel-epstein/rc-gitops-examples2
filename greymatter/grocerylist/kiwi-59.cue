...
Kiwi_59: {
	...
	config: [
		...
		// Kiwi_59 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_59 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-59/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-59$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-59
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-59 proxy object. All configs
