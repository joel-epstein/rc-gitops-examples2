...
Cranberry_89: {
	...
	config: [
		...
		// Cranberry_89 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_89 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-89/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-89$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-89
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-89 proxy object. All configs
