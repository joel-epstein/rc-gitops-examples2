...
Cranberry_20: {
	...
	config: [
		...
		// Cranberry_20 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_20 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-20/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-20$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-20
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-20 proxy object. All configs
