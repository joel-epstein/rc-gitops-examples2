...
Kiwi_976: {
	...
	config: [
		...
		// Kiwi_976 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_976 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_976/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_976$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_976
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_976 proxy object. All configs
