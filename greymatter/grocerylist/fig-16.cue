...
Fig_16: {
	...
	config: [
		...
		// Fig_16 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_16 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-16/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-16$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-16
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-16 proxy object. All configs
