...
Blueberry_67: {
	...
	config: [
		...
		// Blueberry_67 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_67 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-67/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-67$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-67
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-67 proxy object. All configs
