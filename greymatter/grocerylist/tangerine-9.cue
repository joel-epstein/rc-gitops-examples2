...
Tangerine_9: {
	...
	config: [
		...
		// Tangerine_9 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Tangerine_9 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/tangerine-9/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/tangerine-9$"
					to:            route_match.path
		...
		// greymatter catalog service definition for tangerine-9
		#catalog_entry & {
			name:                      Name
		...
		// Shared tangerine-9 proxy object. All configs
