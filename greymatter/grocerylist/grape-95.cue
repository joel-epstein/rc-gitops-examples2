...
Grape_95: {
	...
	config: [
		...
		// Grape_95 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_95 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-95/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-95$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-95
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-95 proxy object. All configs
