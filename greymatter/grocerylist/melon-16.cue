...
Melon_16: {
	...
	config: [
		...
		// Melon_16 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_16 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-16/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-16$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-16
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-16 proxy object. All configs
