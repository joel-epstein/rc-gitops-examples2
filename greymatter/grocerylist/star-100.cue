...
Star_100: {
	...
	config: [
		...
		// Star_100 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_100 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-100/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-100$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-100
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-100 proxy object. All configs
