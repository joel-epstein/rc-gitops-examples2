...
Grape_99: {
	...
	config: [
		...
		// Grape_99 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_99 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-99/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-99$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-99
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-99 proxy object. All configs
