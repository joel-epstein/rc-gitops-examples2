...
Cranberry_14: {
	...
	config: [
		...
		// Cranberry_14 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_14 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-14/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-14$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-14
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-14 proxy object. All configs
