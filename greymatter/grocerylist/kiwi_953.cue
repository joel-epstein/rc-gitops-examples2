...
Kiwi_953: {
	...
	config: [
		...
		// Kiwi_953 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_953 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_953/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_953$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_953
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_953 proxy object. All configs
