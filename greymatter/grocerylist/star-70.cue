...
Star_70: {
	...
	config: [
		...
		// Star_70 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_70 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-70/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-70$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-70
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-70 proxy object. All configs
