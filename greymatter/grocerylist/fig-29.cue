...
Fig_29: {
	...
	config: [
		...
		// Fig_29 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_29 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-29/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-29$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-29
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-29 proxy object. All configs
