...
Kiwi_743: {
	...
	config: [
		...
		// Kiwi_743 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_743 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_743/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_743$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_743
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_743 proxy object. All configs
