...
Cranberry_59: {
	...
	config: [
		...
		// Cranberry_59 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_59 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-59/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-59$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-59
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-59 proxy object. All configs
