...
Grape_61: {
	...
	config: [
		...
		// Grape_61 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_61 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-61/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-61$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-61
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-61 proxy object. All configs
