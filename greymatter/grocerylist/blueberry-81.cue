...
Blueberry_81: {
	...
	config: [
		...
		// Blueberry_81 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_81 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-81/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-81$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-81
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-81 proxy object. All configs
