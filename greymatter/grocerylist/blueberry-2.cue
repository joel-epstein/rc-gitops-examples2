...
Blueberry_2: {
	...
	config: [
		...
		// Blueberry_2 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_2 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-2/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-2$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-2
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-2 proxy object. All configs
