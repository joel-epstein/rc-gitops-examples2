...
Star_62: {
	...
	config: [
		...
		// Star_62 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_62 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-62/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-62$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-62
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-62 proxy object. All configs
