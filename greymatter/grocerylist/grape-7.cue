...
Grape_7: {
	...
	config: [
		...
		// Grape_7 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_7 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-7/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-7$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-7
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-7 proxy object. All configs
