...
Cranberry_35: {
	...
	config: [
		...
		// Cranberry_35 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_35 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-35/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-35$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-35
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-35 proxy object. All configs
