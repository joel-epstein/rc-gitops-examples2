...
Fig_56: {
	...
	config: [
		...
		// Fig_56 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_56 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-56/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-56$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-56
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-56 proxy object. All configs
