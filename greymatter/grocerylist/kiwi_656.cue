...
Kiwi_656: {
	...
	config: [
		...
		// Kiwi_656 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_656 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_656/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_656$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_656
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_656 proxy object. All configs
