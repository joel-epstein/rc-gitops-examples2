...
Star_50: {
	...
	config: [
		...
		// Star_50 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_50 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-50/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-50$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-50
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-50 proxy object. All configs
