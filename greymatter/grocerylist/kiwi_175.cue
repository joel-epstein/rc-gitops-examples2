...
Kiwi_175: {
	...
	config: [
		...
		// Kiwi_175 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_175 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_175/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_175$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_175
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_175 proxy object. All configs
