...
Kiwi_50: {
	...
	config: [
		...
		// Kiwi_50 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_50 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-50/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-50$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-50
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-50 proxy object. All configs
