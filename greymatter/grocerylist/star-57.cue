...
Star_57: {
	...
	config: [
		...
		// Star_57 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_57 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-57/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-57$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-57
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-57 proxy object. All configs
