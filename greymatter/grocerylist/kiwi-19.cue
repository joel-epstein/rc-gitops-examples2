...
Kiwi_19: {
	...
	config: [
		...
		// Kiwi_19 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_19 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-19/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-19$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-19
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-19 proxy object. All configs
