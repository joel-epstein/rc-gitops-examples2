...
Star_61: {
	...
	config: [
		...
		// Star_61 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_61 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-61/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-61$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-61
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-61 proxy object. All configs
