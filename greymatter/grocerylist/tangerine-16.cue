...
Tangerine_16: {
	...
	config: [
		...
		// Tangerine_16 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Tangerine_16 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/tangerine-16/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/tangerine-16$"
					to:            route_match.path
		...
		// greymatter catalog service definition for tangerine-16
		#catalog_entry & {
			name:                      Name
		...
		// Shared tangerine-16 proxy object. All configs
