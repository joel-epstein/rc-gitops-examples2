...
Cranberry_37: {
	...
	config: [
		...
		// Cranberry_37 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_37 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-37/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-37$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-37
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-37 proxy object. All configs
