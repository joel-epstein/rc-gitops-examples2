...
Fig_76: {
	...
	config: [
		...
		// Fig_76 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_76 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-76/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-76$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-76
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-76 proxy object. All configs
