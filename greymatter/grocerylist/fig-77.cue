...
Fig_77: {
	...
	config: [
		...
		// Fig_77 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_77 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-77/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-77$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-77
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-77 proxy object. All configs
