...
Star16: {
	...
	config: [
		...
		// Star16 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star16 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star16/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star16$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star16
		#catalog_entry & {
			name:                      Name
		...
		// Shared star16 proxy object. All configs
