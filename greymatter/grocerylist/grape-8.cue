...
Grape_8: {
	...
	config: [
		...
		// Grape_8 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_8 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-8/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-8$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-8
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-8 proxy object. All configs
