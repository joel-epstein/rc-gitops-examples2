...
Fig_51: {
	...
	config: [
		...
		// Fig_51 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_51 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-51/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-51$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-51
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-51 proxy object. All configs
