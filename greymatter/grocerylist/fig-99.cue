...
Fig_99: {
	...
	config: [
		...
		// Fig_99 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_99 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-99/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-99$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-99
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-99 proxy object. All configs
