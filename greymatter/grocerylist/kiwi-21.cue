...
Kiwi_21: {
	...
	config: [
		...
		// Kiwi_21 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_21 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-21/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-21$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-21
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-21 proxy object. All configs
