...
Star_1: {
	...
	config: [
		...
		// Star_1 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_1 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-1/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-1$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-1
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-1 proxy object. All configs
