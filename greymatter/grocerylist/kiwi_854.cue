...
Kiwi_854: {
	...
	config: [
		...
		// Kiwi_854 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_854 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_854/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_854$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_854
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_854 proxy object. All configs
