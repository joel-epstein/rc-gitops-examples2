...
Star_55: {
	...
	config: [
		...
		// Star_55 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_55 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-55/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-55$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-55
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-55 proxy object. All configs
