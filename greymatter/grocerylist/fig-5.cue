...
Fig_5: {
	...
	config: [
		...
		// Fig_5 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_5 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-5/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-5$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-5
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-5 proxy object. All configs
