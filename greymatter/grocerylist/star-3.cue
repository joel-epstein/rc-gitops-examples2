...
Star_3: {
	...
	config: [
		...
		// Star_3 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_3 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-3/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-3$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-3
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-3 proxy object. All configs
