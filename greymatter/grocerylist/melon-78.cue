...
Melon_78: {
	...
	config: [
		...
		// Melon_78 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_78 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-78/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-78$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-78
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-78 proxy object. All configs
