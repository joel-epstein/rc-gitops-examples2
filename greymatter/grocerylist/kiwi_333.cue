...
Kiwi_333: {
	...
	config: [
		...
		// Kiwi_333 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_333 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_333/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_333$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_333
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_333 proxy object. All configs
