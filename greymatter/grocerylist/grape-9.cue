...
Grape_9: {
	...
	config: [
		...
		// Grape_9 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_9 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-9/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-9$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-9
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-9 proxy object. All configs
