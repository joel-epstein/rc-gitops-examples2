...
Grape_32: {
	...
	config: [
		...
		// Grape_32 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_32 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-32/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-32$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-32
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-32 proxy object. All configs
