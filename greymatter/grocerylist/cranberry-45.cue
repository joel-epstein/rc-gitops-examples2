...
Cranberry_45: {
	...
	config: [
		...
		// Cranberry_45 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_45 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-45/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-45$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-45
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-45 proxy object. All configs
