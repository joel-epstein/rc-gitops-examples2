...
Grape_43: {
	...
	config: [
		...
		// Grape_43 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_43 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-43/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-43$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-43
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-43 proxy object. All configs
