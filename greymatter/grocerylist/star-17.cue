...
Star_17: {
	...
	config: [
		...
		// Star_17 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_17 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-17/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-17$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-17
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-17 proxy object. All configs
