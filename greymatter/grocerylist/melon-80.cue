...
Melon_80: {
	...
	config: [
		...
		// Melon_80 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_80 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-80/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-80$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-80
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-80 proxy object. All configs
