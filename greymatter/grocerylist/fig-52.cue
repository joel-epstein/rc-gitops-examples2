...
Fig_52: {
	...
	config: [
		...
		// Fig_52 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_52 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-52/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-52$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-52
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-52 proxy object. All configs
