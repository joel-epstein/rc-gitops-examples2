...
Cranberry_8: {
	...
	config: [
		...
		// Cranberry_8 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_8 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-8/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-8$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-8
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-8 proxy object. All configs
