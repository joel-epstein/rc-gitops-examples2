...
Kiwi_713: {
	...
	config: [
		...
		// Kiwi_713 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_713 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_713/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_713$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_713
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_713 proxy object. All configs
