...
Star_16: {
	...
	config: [
		...
		// Star_16 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_16 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-16/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-16$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-16
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-16 proxy object. All configs
