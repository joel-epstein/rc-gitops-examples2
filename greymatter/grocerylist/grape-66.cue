...
Grape_66: {
	...
	config: [
		...
		// Grape_66 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_66 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-66/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-66$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-66
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-66 proxy object. All configs
