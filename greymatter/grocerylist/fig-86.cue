...
Fig_86: {
	...
	config: [
		...
		// Fig_86 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_86 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-86/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-86$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-86
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-86 proxy object. All configs
