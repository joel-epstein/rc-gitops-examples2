...
Star_6: {
	...
	config: [
		...
		// Star_6 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_6 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-6/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-6$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-6
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-6 proxy object. All configs
