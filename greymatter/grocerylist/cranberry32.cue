...
Cranberry32: {
	...
	config: [
		...
		// Cranberry32 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry32 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry32/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry32$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry32
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry32 proxy object. All configs
