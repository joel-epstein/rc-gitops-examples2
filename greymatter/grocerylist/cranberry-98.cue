...
Cranberry_98: {
	...
	config: [
		...
		// Cranberry_98 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_98 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-98/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-98$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-98
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-98 proxy object. All configs
