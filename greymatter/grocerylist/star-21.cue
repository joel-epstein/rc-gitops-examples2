...
Star_21: {
	...
	config: [
		...
		// Star_21 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_21 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-21/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-21$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-21
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-21 proxy object. All configs
