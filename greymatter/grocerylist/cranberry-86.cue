...
Cranberry_86: {
	...
	config: [
		...
		// Cranberry_86 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_86 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-86/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-86$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-86
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-86 proxy object. All configs
