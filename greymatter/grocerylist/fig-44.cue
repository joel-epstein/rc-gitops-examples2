...
Fig_44: {
	...
	config: [
		...
		// Fig_44 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_44 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-44/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-44$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-44
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-44 proxy object. All configs
