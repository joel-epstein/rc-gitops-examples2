...
Star_71: {
	...
	config: [
		...
		// Star_71 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_71 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-71/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-71$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-71
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-71 proxy object. All configs
