...
Cranberry_75: {
	...
	config: [
		...
		// Cranberry_75 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_75 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-75/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-75$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-75
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-75 proxy object. All configs
