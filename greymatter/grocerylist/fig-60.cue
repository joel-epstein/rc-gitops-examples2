...
Fig_60: {
	...
	config: [
		...
		// Fig_60 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_60 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-60/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-60$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-60
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-60 proxy object. All configs
