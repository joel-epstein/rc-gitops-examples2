...
Star_7: {
	...
	config: [
		...
		// Star_7 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_7 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-7/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-7$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-7
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-7 proxy object. All configs
