...
Grape_92: {
	...
	config: [
		...
		// Grape_92 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_92 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-92/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-92$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-92
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-92 proxy object. All configs
