...
Cranberry_55: {
	...
	config: [
		...
		// Cranberry_55 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_55 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-55/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-55$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-55
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-55 proxy object. All configs
