...
Star_75: {
	...
	config: [
		...
		// Star_75 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_75 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-75/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-75$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-75
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-75 proxy object. All configs
