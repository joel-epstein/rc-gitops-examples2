...
Blueberry_78: {
	...
	config: [
		...
		// Blueberry_78 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_78 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-78/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-78$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-78
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-78 proxy object. All configs
