...
Blueberry_61: {
	...
	config: [
		...
		// Blueberry_61 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_61 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-61/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-61$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-61
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-61 proxy object. All configs
