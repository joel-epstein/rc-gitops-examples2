...
Fig_34: {
	...
	config: [
		...
		// Fig_34 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_34 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-34/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-34$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-34
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-34 proxy object. All configs
