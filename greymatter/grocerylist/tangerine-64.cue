...
Tangerine_64: {
	...
	config: [
		...
		// Tangerine_64 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Tangerine_64 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/tangerine-64/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/tangerine-64$"
					to:            route_match.path
		...
		// greymatter catalog service definition for tangerine-64
		#catalog_entry & {
			name:                      Name
		...
		// Shared tangerine-64 proxy object. All configs
