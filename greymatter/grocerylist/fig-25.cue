...
Fig_25: {
	...
	config: [
		...
		// Fig_25 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_25 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-25/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-25$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-25
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-25 proxy object. All configs
