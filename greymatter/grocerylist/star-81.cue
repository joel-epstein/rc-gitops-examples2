...
Star_81: {
	...
	config: [
		...
		// Star_81 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_81 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-81/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-81$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-81
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-81 proxy object. All configs
