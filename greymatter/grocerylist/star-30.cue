...
Star_30: {
	...
	config: [
		...
		// Star_30 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_30 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-30/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-30$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-30
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-30 proxy object. All configs
