...
Cranberry_54: {
	...
	config: [
		...
		// Cranberry_54 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_54 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-54/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-54$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-54
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-54 proxy object. All configs
