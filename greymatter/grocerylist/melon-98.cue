...
Melon_98: {
	...
	config: [
		...
		// Melon_98 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_98 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-98/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-98$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-98
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-98 proxy object. All configs
