...
Cranberry_27: {
	...
	config: [
		...
		// Cranberry_27 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_27 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-27/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-27$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-27
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-27 proxy object. All configs
