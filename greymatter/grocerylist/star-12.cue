...
Star_12: {
	...
	config: [
		...
		// Star_12 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_12 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-12/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-12$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-12
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-12 proxy object. All configs
