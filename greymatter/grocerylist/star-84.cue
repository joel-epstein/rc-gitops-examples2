...
Star_84: {
	...
	config: [
		...
		// Star_84 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_84 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-84/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-84$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-84
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-84 proxy object. All configs
