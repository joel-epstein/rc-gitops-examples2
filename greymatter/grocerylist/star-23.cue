...
Star_23: {
	...
	config: [
		...
		// Star_23 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_23 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-23/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-23$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-23
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-23 proxy object. All configs
