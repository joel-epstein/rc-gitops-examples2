...
Grape_60: {
	...
	config: [
		...
		// Grape_60 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_60 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-60/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-60$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-60
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-60 proxy object. All configs
