...
Fig_49: {
	...
	config: [
		...
		// Fig_49 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_49 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-49/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-49$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-49
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-49 proxy object. All configs
