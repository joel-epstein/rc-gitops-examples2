...
Cranberry_62: {
	...
	config: [
		...
		// Cranberry_62 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_62 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-62/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-62$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-62
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-62 proxy object. All configs
