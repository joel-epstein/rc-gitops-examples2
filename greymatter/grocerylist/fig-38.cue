...
Fig_38: {
	...
	config: [
		...
		// Fig_38 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_38 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-38/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-38$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-38
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-38 proxy object. All configs
