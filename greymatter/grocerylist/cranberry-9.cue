...
Cranberry_9: {
	...
	config: [
		...
		// Cranberry_9 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_9 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-9/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-9$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-9
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-9 proxy object. All configs
