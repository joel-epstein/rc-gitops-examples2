...
Fig_70: {
	...
	config: [
		...
		// Fig_70 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_70 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-70/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-70$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-70
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-70 proxy object. All configs
