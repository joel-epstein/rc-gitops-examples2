...
Melon_9: {
	...
	config: [
		...
		// Melon_9 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_9 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-9/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-9$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-9
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-9 proxy object. All configs
