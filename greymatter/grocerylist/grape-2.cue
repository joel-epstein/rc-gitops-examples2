...
Grape_2: {
	...
	config: [
		...
		// Grape_2 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_2 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-2/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-2$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-2
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-2 proxy object. All configs
