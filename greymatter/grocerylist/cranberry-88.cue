...
Cranberry_88: {
	...
	config: [
		...
		// Cranberry_88 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_88 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-88/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-88$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-88
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-88 proxy object. All configs
