...
Grape_10: {
	...
	config: [
		...
		// Grape_10 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_10 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-10/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-10$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-10
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-10 proxy object. All configs
