...
Kiwi_647: {
	...
	config: [
		...
		// Kiwi_647 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_647 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_647/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_647$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_647
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_647 proxy object. All configs
