...
Cranberry_6: {
	...
	config: [
		...
		// Cranberry_6 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_6 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-6/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-6$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-6
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-6 proxy object. All configs
