...
Fig_27: {
	...
	config: [
		...
		// Fig_27 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_27 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-27/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-27$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-27
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-27 proxy object. All configs
