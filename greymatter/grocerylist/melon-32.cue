...
Melon_32: {
	...
	config: [
		...
		// Melon_32 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_32 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-32/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-32$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-32
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-32 proxy object. All configs
