...
Orange_1: {
	...
	config: [
		...
		// Orange_1 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Orange_1 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/orange-1/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/orange-1$"
					to:            route_match.path
		...
		// greymatter catalog service definition for orange-1
		#catalog_entry & {
			name:                      Name
		...
		// Shared orange-1 proxy object. All configs
