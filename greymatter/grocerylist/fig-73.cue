...
Fig_73: {
	...
	config: [
		...
		// Fig_73 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_73 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-73/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-73$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-73
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-73 proxy object. All configs
