...
Cranberry_46: {
	...
	config: [
		...
		// Cranberry_46 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_46 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-46/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-46$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-46
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-46 proxy object. All configs
