...
Fig_24: {
	...
	config: [
		...
		// Fig_24 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_24 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-24/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-24$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-24
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-24 proxy object. All configs
