...
Cranberry_44: {
	...
	config: [
		...
		// Cranberry_44 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_44 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-44/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-44$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-44
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-44 proxy object. All configs
