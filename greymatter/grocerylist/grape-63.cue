...
Grape_63: {
	...
	config: [
		...
		// Grape_63 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_63 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-63/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-63$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-63
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-63 proxy object. All configs
