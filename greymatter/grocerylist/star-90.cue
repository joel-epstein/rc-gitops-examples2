...
Star_90: {
	...
	config: [
		...
		// Star_90 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_90 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-90/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-90$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-90
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-90 proxy object. All configs
