...
Fig_92: {
	...
	config: [
		...
		// Fig_92 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_92 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-92/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-92$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-92
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-92 proxy object. All configs
