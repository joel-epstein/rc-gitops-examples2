...
Kiwi_387: {
	...
	config: [
		...
		// Kiwi_387 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_387 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_387/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_387$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_387
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_387 proxy object. All configs
