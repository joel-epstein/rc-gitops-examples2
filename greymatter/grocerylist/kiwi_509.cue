...
Kiwi_509: {
	...
	config: [
		...
		// Kiwi_509 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_509 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_509/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_509$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_509
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_509 proxy object. All configs
