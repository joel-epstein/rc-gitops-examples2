...
Fig_17: {
	...
	config: [
		...
		// Fig_17 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_17 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-17/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-17$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-17
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-17 proxy object. All configs
