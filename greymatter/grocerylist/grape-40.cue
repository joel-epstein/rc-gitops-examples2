...
Grape_40: {
	...
	config: [
		...
		// Grape_40 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_40 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-40/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-40$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-40
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-40 proxy object. All configs
