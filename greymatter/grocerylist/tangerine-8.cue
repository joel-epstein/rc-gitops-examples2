...
Tangerine_8: {
	...
	config: [
		...
		// Tangerine_8 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Tangerine_8 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/tangerine-8/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/tangerine-8$"
					to:            route_match.path
		...
		// greymatter catalog service definition for tangerine-8
		#catalog_entry & {
			name:                      Name
		...
		// Shared tangerine-8 proxy object. All configs
