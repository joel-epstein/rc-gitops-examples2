...
Kiwi_9: {
	...
	config: [
		...
		// Kiwi_9 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_9 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-9/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-9$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-9
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-9 proxy object. All configs
