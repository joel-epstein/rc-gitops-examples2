...
Fig_39: {
	...
	config: [
		...
		// Fig_39 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_39 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-39/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-39$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-39
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-39 proxy object. All configs
