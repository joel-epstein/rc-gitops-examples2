...
Melon_4: {
	...
	config: [
		...
		// Melon_4 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_4 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-4/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-4$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-4
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-4 proxy object. All configs
