...
Cranberry_17: {
	...
	config: [
		...
		// Cranberry_17 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_17 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-17/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-17$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-17
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-17 proxy object. All configs
