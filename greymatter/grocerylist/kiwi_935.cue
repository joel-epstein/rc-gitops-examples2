...
Kiwi_935: {
	...
	config: [
		...
		// Kiwi_935 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_935 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_935/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_935$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_935
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_935 proxy object. All configs
