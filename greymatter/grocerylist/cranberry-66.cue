...
Cranberry_66: {
	...
	config: [
		...
		// Cranberry_66 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_66 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-66/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-66$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-66
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-66 proxy object. All configs
