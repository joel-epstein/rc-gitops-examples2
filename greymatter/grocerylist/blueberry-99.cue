...
Blueberry_99: {
	...
	config: [
		...
		// Blueberry_99 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_99 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-99/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-99$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-99
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-99 proxy object. All configs
