...
Melon_88: {
	...
	config: [
		...
		// Melon_88 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_88 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-88/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-88$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-88
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-88 proxy object. All configs
