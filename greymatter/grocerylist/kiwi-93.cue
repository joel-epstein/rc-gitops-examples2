...
Kiwi_93: {
	...
	config: [
		...
		// Kiwi_93 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_93 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-93/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-93$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-93
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-93 proxy object. All configs
