...
Kiwi_511: {
	...
	config: [
		...
		// Kiwi_511 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_511 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_511/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_511$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_511
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_511 proxy object. All configs
