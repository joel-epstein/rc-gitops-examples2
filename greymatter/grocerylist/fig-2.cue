...
Fig_2: {
	...
	config: [
		...
		// Fig_2 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_2 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-2/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-2$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-2
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-2 proxy object. All configs
