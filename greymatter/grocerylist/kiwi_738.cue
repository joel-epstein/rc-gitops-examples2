...
Kiwi_738: {
	...
	config: [
		...
		// Kiwi_738 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_738 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_738/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_738$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_738
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_738 proxy object. All configs
