...
Cranberry_56: {
	...
	config: [
		...
		// Cranberry_56 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_56 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-56/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-56$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-56
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-56 proxy object. All configs
