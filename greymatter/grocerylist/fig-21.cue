...
Fig_21: {
	...
	config: [
		...
		// Fig_21 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_21 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-21/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-21$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-21
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-21 proxy object. All configs
