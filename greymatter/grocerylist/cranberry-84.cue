...
Cranberry_84: {
	...
	config: [
		...
		// Cranberry_84 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_84 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-84/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-84$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-84
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-84 proxy object. All configs
