...
Kiwi_27: {
	...
	config: [
		...
		// Kiwi_27 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_27 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-27/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-27$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-27
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-27 proxy object. All configs
