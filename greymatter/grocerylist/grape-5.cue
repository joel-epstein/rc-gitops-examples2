...
Grape_5: {
	...
	config: [
		...
		// Grape_5 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_5 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-5/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-5$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-5
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-5 proxy object. All configs
