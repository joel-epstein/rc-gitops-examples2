...
Cranberry_11: {
	...
	config: [
		...
		// Cranberry_11 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_11 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-11/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-11$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-11
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-11 proxy object. All configs
