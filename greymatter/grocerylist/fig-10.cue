...
Fig_10: {
	...
	config: [
		...
		// Fig_10 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_10 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-10/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-10$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-10
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-10 proxy object. All configs
