...
Fig_36: {
	...
	config: [
		...
		// Fig_36 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_36 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-36/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-36$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-36
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-36 proxy object. All configs
