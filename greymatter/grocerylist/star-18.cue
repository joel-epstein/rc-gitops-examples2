...
Star_18: {
	...
	config: [
		...
		// Star_18 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_18 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-18/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-18$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-18
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-18 proxy object. All configs
