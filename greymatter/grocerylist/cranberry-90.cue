...
Cranberry_90: {
	...
	config: [
		...
		// Cranberry_90 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_90 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-90/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-90$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-90
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-90 proxy object. All configs
