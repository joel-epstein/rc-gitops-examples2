...
Melon_31: {
	...
	config: [
		...
		// Melon_31 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Melon_31 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/melon-31/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/melon-31$"
					to:            route_match.path
		...
		// greymatter catalog service definition for melon-31
		#catalog_entry & {
			name:                      Name
		...
		// Shared melon-31 proxy object. All configs
