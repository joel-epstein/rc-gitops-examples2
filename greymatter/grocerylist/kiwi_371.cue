...
Kiwi_371: {
	...
	config: [
		...
		// Kiwi_371 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_371 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_371/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_371$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_371
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_371 proxy object. All configs
