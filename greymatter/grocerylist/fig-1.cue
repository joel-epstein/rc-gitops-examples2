...
Fig_1: {
	...
	config: [
		...
		// Fig_1 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_1 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-1/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-1$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-1
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-1 proxy object. All configs
