...
Fig_12: {
	...
	config: [
		...
		// Fig_12 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_12 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-12/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-12$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-12
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-12 proxy object. All configs
