...
Kiwi_2: {
	...
	config: [
		...
		// Kiwi_2 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_2 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-2/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-2$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-2
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-2 proxy object. All configs
