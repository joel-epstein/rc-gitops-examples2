...
Cranberry_74: {
	...
	config: [
		...
		// Cranberry_74 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_74 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-74/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-74$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-74
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-74 proxy object. All configs
