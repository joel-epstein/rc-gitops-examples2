...
Grape_45: {
	...
	config: [
		...
		// Grape_45 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_45 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-45/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-45$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-45
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-45 proxy object. All configs
