...
Star_91: {
	...
	config: [
		...
		// Star_91 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_91 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-91/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-91$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-91
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-91 proxy object. All configs
