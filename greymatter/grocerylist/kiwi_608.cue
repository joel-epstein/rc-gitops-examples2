...
Kiwi_608: {
	...
	config: [
		...
		// Kiwi_608 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_608 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_608/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_608$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_608
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_608 proxy object. All configs
