...
Cranberry_3: {
	...
	config: [
		...
		// Cranberry_3 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_3 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-3/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-3$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-3
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-3 proxy object. All configs
