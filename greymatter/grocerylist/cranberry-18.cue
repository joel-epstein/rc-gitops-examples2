...
Cranberry_18: {
	...
	config: [
		...
		// Cranberry_18 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_18 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-18/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-18$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-18
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-18 proxy object. All configs
