...
Grape_88: {
	...
	config: [
		...
		// Grape_88 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_88 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-88/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-88$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-88
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-88 proxy object. All configs
