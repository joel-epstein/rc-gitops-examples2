...
Cranberry_87: {
	...
	config: [
		...
		// Cranberry_87 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_87 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-87/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-87$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-87
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-87 proxy object. All configs
