...
Star_15: {
	...
	config: [
		...
		// Star_15 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_15 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-15/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-15$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-15
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-15 proxy object. All configs
