...
Star25: {
	...
	config: [
		...
		// Star25 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star25 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star25/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star25$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star25
		#catalog_entry & {
			name:                      Name
		...
		// Shared star25 proxy object. All configs
