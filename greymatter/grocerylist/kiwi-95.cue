...
Kiwi_95: {
	...
	config: [
		...
		// Kiwi_95 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_95 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi-95/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi-95$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi-95
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi-95 proxy object. All configs
