...
Star_56: {
	...
	config: [
		...
		// Star_56 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_56 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-56/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-56$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-56
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-56 proxy object. All configs
