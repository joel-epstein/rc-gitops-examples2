...
Tangerine_2: {
	...
	config: [
		...
		// Tangerine_2 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Tangerine_2 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/tangerine-2/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/tangerine-2$"
					to:            route_match.path
		...
		// greymatter catalog service definition for tangerine-2
		#catalog_entry & {
			name:                      Name
		...
		// Shared tangerine-2 proxy object. All configs
