...
Fig_37: {
	...
	config: [
		...
		// Fig_37 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_37 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-37/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-37$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-37
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-37 proxy object. All configs
