...
Blueberry_7: {
	...
	config: [
		...
		// Blueberry_7 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_7 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-7/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-7$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-7
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-7 proxy object. All configs
