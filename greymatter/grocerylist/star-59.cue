...
Star_59: {
	...
	config: [
		...
		// Star_59 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_59 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-59/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-59$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-59
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-59 proxy object. All configs
