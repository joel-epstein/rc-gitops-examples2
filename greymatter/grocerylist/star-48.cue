...
Star_48: {
	...
	config: [
		...
		// Star_48 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_48 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-48/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-48$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-48
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-48 proxy object. All configs
