...
Fig_22: {
	...
	config: [
		...
		// Fig_22 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_22 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-22/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-22$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-22
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-22 proxy object. All configs
