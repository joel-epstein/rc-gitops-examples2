...
Blueberry_100: {
	...
	config: [
		...
		// Blueberry_100 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_100 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-100/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-100$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-100
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-100 proxy object. All configs
