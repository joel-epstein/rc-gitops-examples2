...
Cranberry_33: {
	...
	config: [
		...
		// Cranberry_33 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_33 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-33/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-33$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-33
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-33 proxy object. All configs
