...
Kiwi_866: {
	...
	config: [
		...
		// Kiwi_866 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_866 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_866/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_866$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_866
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_866 proxy object. All configs
