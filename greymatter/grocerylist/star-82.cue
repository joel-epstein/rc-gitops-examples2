...
Star_82: {
	...
	config: [
		...
		// Star_82 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_82 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-82/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-82$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-82
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-82 proxy object. All configs
