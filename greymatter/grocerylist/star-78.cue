...
Star_78: {
	...
	config: [
		...
		// Star_78 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_78 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-78/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-78$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-78
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-78 proxy object. All configs
