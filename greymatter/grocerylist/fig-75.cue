...
Fig_75: {
	...
	config: [
		...
		// Fig_75 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_75 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-75/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-75$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-75
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-75 proxy object. All configs
