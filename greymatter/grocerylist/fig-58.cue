...
Fig_58: {
	...
	config: [
		...
		// Fig_58 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_58 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-58/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-58$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-58
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-58 proxy object. All configs
