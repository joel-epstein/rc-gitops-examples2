...
Star20: {
	...
	config: [
		...
		// Star20 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star20 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star20/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star20$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star20
		#catalog_entry & {
			name:                      Name
		...
		// Shared star20 proxy object. All configs
