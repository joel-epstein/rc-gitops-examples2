...
Blueberry_59: {
	...
	config: [
		...
		// Blueberry_59 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_59 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-59/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-59$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-59
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-59 proxy object. All configs
