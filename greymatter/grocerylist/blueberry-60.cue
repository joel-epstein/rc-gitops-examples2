...
Blueberry_60: {
	...
	config: [
		...
		// Blueberry_60 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Blueberry_60 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/blueberry-60/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/blueberry-60$"
					to:            route_match.path
		...
		// greymatter catalog service definition for blueberry-60
		#catalog_entry & {
			name:                      Name
		...
		// Shared blueberry-60 proxy object. All configs
