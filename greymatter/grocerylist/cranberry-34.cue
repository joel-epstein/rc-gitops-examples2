...
Cranberry_34: {
	...
	config: [
		...
		// Cranberry_34 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_34 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-34/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-34$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-34
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-34 proxy object. All configs
