...
Star_67: {
	...
	config: [
		...
		// Star_67 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_67 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-67/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-67$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-67
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-67 proxy object. All configs
