...
Cranberry_1: {
	...
	config: [
		...
		// Cranberry_1 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_1 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-1/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-1$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-1
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-1 proxy object. All configs
