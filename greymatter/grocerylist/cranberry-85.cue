...
Cranberry_85: {
	...
	config: [
		...
		// Cranberry_85 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Cranberry_85 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/cranberry-85/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/cranberry-85$"
					to:            route_match.path
		...
		// greymatter catalog service definition for cranberry-85
		#catalog_entry & {
			name:                      Name
		...
		// Shared cranberry-85 proxy object. All configs
