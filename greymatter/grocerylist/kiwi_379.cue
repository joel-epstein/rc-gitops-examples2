...
Kiwi_379: {
	...
	config: [
		...
		// Kiwi_379 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Kiwi_379 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/kiwi_379/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/kiwi_379$"
					to:            route_match.path
		...
		// greymatter catalog service definition for kiwi_379
		#catalog_entry & {
			name:                      Name
		...
		// Shared kiwi_379 proxy object. All configs
